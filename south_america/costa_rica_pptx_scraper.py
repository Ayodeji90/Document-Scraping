#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Costa Rica-focused PPTX scraper — Search-Engine-Driven (South America Series)"""
...
TOPICS: List[str] = ['diapositivas', 'presentación', 'material de clase', 'seminario', 'ingeniería', 'física', 'química', 'biología', 'matemáticas', 'medicina', 'economía', 'derecho']

SITE_DOMAINS: List[str] = ['site:ucr.ac.cr', 'site:tec.ac.cr', 'site:una.ac.cr', 'site:uned.ac.cr', 'site:ulacit.ac.cr', 'site:uia.ac.cr']
...
def build_query_list() -> List[str]:
    queries = []
    for b in ['site:ac.cr', 'site:cr', 'site:ucr.ac.cr', 'site:tec.ac.cr']:
...
class CostaRicaScraper:
    def __init__(self, out_dir="downloaded_ppts_costa_rica", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 CR_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Costa Rica Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Costa Rica Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    CostaRicaScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
