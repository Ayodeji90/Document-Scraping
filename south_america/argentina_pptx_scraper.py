...
# -*- coding: utf-8 -*-
"""
Argentina-focused PPTX scraper — Search-Engine-Driven (South America Series)
Discovers and downloads academic .pptx / .ppt files from Argentine institutions (.ar).

Strategies for 10,000+ files:
  1. Priority focus on UBA, La Plata (UNLP), and Córdoba (UNC).
  2. Argentine Spanish academic keywords (Clase, Slides, Cátedra).
...
# Argentine repositories often use DSpace or custom portal systems
...
# Argentine Academic Topics & Document Types (Spanish + English)
TOPICS: List[str] = [
    # Document Types
    ...
    "física", "mecánica cuántica", "termodinâmica", "óptica",
    "química", "química orgánica", "bioquímica",
    "biología", "genética", "biotecnología",
    "matemática", "estadística", "álgebra", "análisis",
    "medicina", "anatomía", "fisiología", "cirugía",
    "economía", "gestión", "marketing", "finanzas",
    "historia", "filosofía", "psicologia", "sociología", "derecho",
    "arquitectura", "diseño", "urbanismo"
]

# Major Argentine Academic Domains
SITE_DOMAINS: List[str] = [
    "site:uba.ar",           # University of Buenos Aires
    "site:unlp.edu.ar",      # La Plata
    "site:unc.edu.ar",       # Córdoba
    "site:unr.edu.ar",       # Rosario
    "site:utn.edu.ar",       # National Technological University
    "site:itba.edu.ar",      # ITBA (Tech focus)
    "site:udesa.edu.ar",     # San Andrés
    "site:utdt.edu.ar",      # Torcuato Di Tella
    "site:uns.edu.ar",       # Sur
    "site:conicet.gov.ar"    # CONICET (National Research Council)
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for Argentina."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:uba.ar", "site:unlp.edu.ar", "site:ar"]
    for b in broad:
...
class ArgentinaScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_argentina",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 AR_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Argentina Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Argentina Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = ArgentinaScraper(verify_ssl=not args.no_verify_ssl)
