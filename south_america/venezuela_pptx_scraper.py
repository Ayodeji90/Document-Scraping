#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Venezuela-focused PPTX scraper — Search-Engine-Driven (South America Series)"""
...
TOPICS: List[str] = ['diapositivas', 'presentación', 'material de clase', 'clase magistral', 'seminario', 'ingeniería', 'física', 'química', 'biología', 'matemáticas', 'medicina', 'economía', 'derecho']

SITE_DOMAINS: List[str] = ['site:ucv.ve', 'site:usb.ve', 'site:uc.edu.ve', 'site:luz.edu.ve', 'site:ula.ve', 'site:unexpo.edu.ve', 'site:unimet.edu.ve', 'site:ucab.edu.ve', 'site:unet.edu.ve', 'site:uneg.edu.ve']
...
def build_query_list() -> List[str]:
    queries = []
    for b in ['site:edu.ve', 'site:ve', 'site:ucv.ve', 'site:usb.ve']:
...
class VenezuelaScraper:
    def __init__(self, out_dir="downloaded_ppts_venezuela", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 VE_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Venezuela Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Venezuela Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    VenezuelaScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
