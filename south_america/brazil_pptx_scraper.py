...
# -*- coding: utf-8 -*-
"""
Brazil-focused PPTX scraper — Search-Engine-Driven (South America Series)
Discovers and downloads academic .pptx / .ppt files from Brazilian institutions (.br).

Strategies for 10,000+ files:
  1. Priority focus on USP, Unicamp, and Federal Universities (UFRJ, UFMG, etc.).
  2. Brazilian Portuguese academic keywords (Aula, Slides, Apresentação).
...
# Brazilian repositories often use DSpace or customized library systems
BITSTREAM_RE = re.compile(
    r"/(?:bitstream(?:/handle)?/[\d./]+|retrieve/\d+|download/\d+|file|anexo|get)/[^\"\'\s>{}\[\]\\]+\.pptx?",
    re.IGNORECASE,
)
...
# Brazilian Academic Topics & Document Types (Portuguese + English)
TOPICS: List[str] = [
    # Document Types
    "aula", "slides", "apresentação", "apostila", "curso", "seminário",
    "lecture slides", "presentation", "research presentation",
    
    # Specific Subjects
    "computação", "inteligência artificial", "aprendizado de máquina",
    "engenharia", "mecânica", "elétrica", "civil",
    "física", "mecânica quântica", "termodinâmica", "ótica",
    "química", "química orgânica", "bioquímica",
    "biologia", "genética", "biotecnologia",
    "matemática", "estatística", "álgebra", "análise",
    "medicina", "anatomia", "fisiologia", "cirurgia",
    "economia", "gestão", "marketing", "finanças",
    "história", "filosofia", "psicologia", "sociologia", "direito",
    "arquitetura", "design", "urbanismo"
]

# Major Brazilian Academic Domains
SITE_DOMAINS: List[str] = [
    "site:usp.br",           # University of São Paulo
    "site:unicamp.br",       # University of Campinas
    "site:unesp.br",         # São Paulo State University
    "site:ufrj.br",          # Federal University of Rio de Janeiro
    "site:ufmg.br",          # Federal University of Minas Gerais
    "site:ufrgs.br",         # Federal University of Rio Grande do Sul
    "site:unifesp.br",       # Federal University of São Paulo
    "site:unb.br",           # University of Brasília
    "site:ufsc.br",          # Federal University of Santa Catarina
    "site:capes.gov.br"      # CAPES (Government research portal)
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for Brazil."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:usp.br", "site:ufrj.br", "site:br"]
    for b in broad:
        queries.append(f"filetype:pptx {b}")
        queries.append(f"filetype:ppt {b}")
        queries.append(f"aula slides filetype:pptx {b}")
...
class BrazilScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_brazil",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 BR_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Brazil Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Brazil Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = BrazilScraper(verify_ssl=not args.no_verify_ssl)
