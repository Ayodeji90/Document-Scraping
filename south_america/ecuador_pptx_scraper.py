#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Ecuador-focused PPTX scraper — Search-Engine-Driven (South America Series)"""
...
TOPICS: List[str] = ['diapositivas', 'presentación', 'material de clase', 'clase magistral', 'seminario', 'lecture slides', 'presentation', 'course materials', 'ciencias de la computación', 'ingeniería', 'física', 'química', 'biología', 'matemáticas', 'medicina', 'economía', 'derecho', 'computer science', 'engineering', 'physics', 'chemistry', 'mathematics']

SITE_DOMAINS: List[str] = ['site:epn.edu.ec', 'site:uce.edu.ec', 'site:espol.edu.ec', 'site:puce.edu.ec', 'site:usfq.edu.ec', 'site:ucuenca.edu.ec', 'site:utpl.edu.ec', 'site:uta.edu.ec', 'site:uees.edu.ec', 'site:udla.edu.ec']

def build_query_list() -> List[str]:
    queries = []
    for b in ['site:edu.ec', 'site:epn.edu.ec', 'site:espol.edu.ec', 'site:ec']:
...
class EcuadorScraper:
    def __init__(self, out_dir="downloaded_ppts_ecuador", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 EC_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Ecuador Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Ecuador Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    EcuadorScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
