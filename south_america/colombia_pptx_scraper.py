#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Colombia-focused PPTX scraper — Search-Engine-Driven (South America Series)"""
...
TOPICS: List[str] = ['diapositivas', 'presentación', 'material de curso', 'clase magistral', 'seminario', 'lecture slides', 'presentation', 'course materials', 'ciencias de la computación', 'inteligencia artificial', 'ingeniería', 'ingeniería eléctrica', 'ingeniería civil', 'física', 'química', 'biología', 'matemáticas', 'medicina', 'economía', 'derecho', 'computer science', 'engineering', 'physics', 'chemistry', 'mathematics']

SITE_DOMAINS: List[str] = ['site:unal.edu.co', 'site:uniandes.edu.co', 'site:javeriana.edu.co', 'site:udea.edu.co', 'site:univalle.edu.co', 'site:urosario.edu.co', 'site:uninorte.edu.co', 'site:icesi.edu.co', 'site:eafit.edu.co', 'site:utp.edu.co']

def build_query_list() -> List[str]:
    queries = []
    for b in ['site:edu.co', 'site:unal.edu.co', 'site:uniandes.edu.co', 'site:co']:
        queries.append(f"filetype:pptx {b}")
        queries.append(f"filetype:ppt {b}")
        queries.append(f"diapositivas presentación filetype:pptx {b}")
    remaining = []
...
class ColombiaScraper:
    def __init__(self, out_dir="downloaded_ppts_colombia", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 CO_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Colombia Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Colombia Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    ColombiaScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
