#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Peru-focused PPTX scraper — Search-Engine-Driven (South America Series)"""
...
TOPICS: List[str] = ['diapositivas', 'presentación', 'material de clase', 'clase magistral', 'seminario', 'lecture slides', 'presentation', 'course materials', 'ciencias de la computación', 'inteligencia artificial', 'ingeniería', 'física', 'química', 'biología', 'matemáticas', 'medicina', 'economía', 'derecho', 'computer science', 'engineering', 'physics', 'chemistry', 'mathematics', 'medicine']

SITE_DOMAINS: List[str] = ['site:pucp.edu.pe', 'site:uni.edu.pe', 'site:unmsm.edu.pe', 'site:ulima.edu.pe', 'site:upc.edu.pe', 'site:unsa.edu.pe', 'site:unt.edu.pe', 'site:unsaac.edu.pe', 'site:ucsm.edu.pe', 'site:upch.edu.pe']

def build_query_list() -> List[str]:
    queries = []
    for b in ['site:edu.pe', 'site:pucp.edu.pe', 'site:uni.edu.pe', 'site:pe']:
...
class PeruScraper:
    def __init__(self, out_dir="downloaded_ppts_peru", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 PE_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Peru Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Peru Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    PeruScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
