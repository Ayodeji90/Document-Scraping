...
# -*- coding: utf-8 -*-
"""
Chile-focused PPTX scraper — Search-Engine-Driven (South America Series)
Discovers and downloads academic .pptx / .ppt files from Chilean institutions (.cl).

Strategies for 10,000+ files:
  1. Priority focus on University of Chile, PUC, and USACH.
  2. Chilean Spanish academic keywords (Clase, Slides, Cátedra).
...
# Chilean repositories often use DSpace or custom portal systems
BITSTREAM_RE = re.compile(
    r"/(?:bitstream(?:/handle)?/[\d./]+|retrieve/\d+|download/\d+|file|attachment|get|descargar)/[^\"\'\s>{}\[\]\\]+\.pptx?",
    re.IGNORECASE,
)
...
# Chilean Academic Topics & Document Types (Spanish + English)
TOPICS: List[str] = [
    # Document Types
    "clase", "slides", "presentación", "apuntes", "cátedra", "seminario",
    "lecture slides", "presentation", "research presentation",
    
    # Specific Subjects
    "computación", "inteligencia artificial", "aprendizaje automático",
    ...
    "matemática", "estatística", "álgebra", "análisis",
    "medicina", "anatomía", "fisiologia", "cirugia",
    "economía", "gestión", "marketing", "finanzas",
    "história", "filosofia", "psicologia", "sociologia", "derecho",
    "arquitectura", "diseño", "urbanismo"
]

# Major Chilean Academic Domains
SITE_DOMAINS: List[str] = [
    "site:uchile.cl",        # University of Chile
    "site:uc.cl",            # Pontificia Universidad Católica de Chile
    "site:usach.cl",         # University of Santiago, Chile
    "site:udec.cl",          # University of Concepción
    "site:pucv.cl",          # Pontificia Universidad Católica de Valparaíso
    "site:utfsm.cl",         # Federico Santa María Technical University
    "site:uach.cl",          # Austral University of Chile
    "site:uantof.cl",        # University of Antofagasta
    "site:utem.cl",          # Metropolitan University of Technology
    "site:conicyt.cl"        # CONICYT (National Research Council)
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for Chile."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:uchile.cl", "site:uc.cl", "site:cl"]
    for b in broad:
        queries.append(f"filetype:pptx {b}")
        queries.append(f"filetype:ppt {b}")
        queries.append(f"clase slides filetype:pptx {b}")
...
class ChileScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_chile",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 CL_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Chile Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Chile Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = ChileScraper(verify_ssl=not args.no_verify_ssl)
