#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Cuba-focused PPTX scraper — Search-Engine-Driven (South America Series)"""
...
TOPICS: List[str] = ['diapositivas', 'presentación', 'material de clase', 'conferencia', 'ingeniería', 'física', 'química', 'biología', 'matemáticas', 'medicina', 'economía']

SITE_DOMAINS: List[str] = ['site:uh.cu', 'site:cujae.edu.cu', 'site:uclv.edu.cu', 'site:uo.edu.cu', 'site:ucm.edu.cu', 'site:upr.edu.cu', 'site:instec.cu']
...
def build_query_list() -> List[str]:
    queries = []
    for b in ['site:cu', 'site:edu.cu', 'site:uh.cu', 'site:cujae.edu.cu']:
...
class CubaScraper:
    def __init__(self, out_dir="downloaded_ppts_cuba", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 CU_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Cuba Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Cuba Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    CubaScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
