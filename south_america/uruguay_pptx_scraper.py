#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Uruguay-focused PPTX scraper — Search-Engine-Driven (South America Series)"""
...
SITE_DOMAINS: List[str] = ['site:udelar.edu.uy', 'site:ort.edu.uy', 'site:um.edu.uy', 'site:ucudal.edu.uy', 'site:uni.edu.uy', 'site:claeh.edu.uy']
...
def build_query_list() -> List[str]:
    queries = []
    for b in ['site:edu.uy', 'site:uy', 'site:udelar.edu.uy', 'site:ort.edu.uy']:
...
class UruguayScraper:
    def __init__(self, out_dir="downloaded_ppts_uruguay", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 UY_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Uruguay Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Uruguay Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    UruguayScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
