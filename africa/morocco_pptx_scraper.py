#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Morocco-focused PPTX scraper — Search-Engine-Driven (Africa Series)"""
...
TOPICS: List[str] = ['cours magistral', 'diaporama', 'présentation', 'matériel pédagogique', 'محاضرة', 'شرائح', 'informatique', 'ingénierie', 'physique', 'chimie', 'mathématiques', 'médecine', 'économie']

SITE_DOMAINS: List[str] = ['site:um5.ac.ma', 'site:uca.ac.ma', 'site:um6p.ma', 'site:uir.ac.ma', 'site:uae.ac.ma', 'site:umi.ac.ma', 'site:univ-oujda.ac.ma', 'site:uit.ac.ma', 'site:usms.ac.ma', 'site:usmba.ac.ma']
...
def build_query_list() -> List[str]:
    queries = []
    for b in ['site:ac.ma', 'site:ma', 'site:um5.ac.ma', 'site:uca.ac.ma']:
...
class MoroccoScraper:
    def __init__(self, out_dir="downloaded_ppts_morocco", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 MA_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Morocco Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Morocco Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    MoroccoScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
