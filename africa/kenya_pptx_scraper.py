#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Kenya-focused PPTX scraper — Search-Engine-Driven (Africa Series)"""
...
TOPICS: List[str] = [
    ...
    "economics", "business", "finance", "accounting", "agriculture",
    "environmental science", "geography", "political science", "law", "education",
]

SITE_DOMAINS: List[str] = [
    "site:uonbi.ac.ke",        # University of Nairobi
    "site:ku.ac.ke",           # Kenyatta University
    "site:jkuat.ac.ke",        # JKUAT
    "site:moi.ac.ke",          # Moi University
    "site:egerton.ac.ke",      # Egerton University
    "site:strathmore.edu",     # Strathmore University
    "site:usiu.ac.ke",         # USIU-Africa
    "site:mmust.ac.ke",        # Masinde Muliro University
    "site:dkut.ac.ke",         # Dedan Kimathi University
    "site:tuk.ac.ke",          # Technical University of Kenya
]

def build_query_list() -> List[str]:
    queries = []
    for b in ["site:ac.ke", "site:edu.ke", "site:uonbi.ac.ke", "site:ke"]:
...
class KenyaScraper:
    def __init__(self, out_dir="downloaded_ppts_kenya", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 KE_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Kenya Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Kenya Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    KenyaScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
