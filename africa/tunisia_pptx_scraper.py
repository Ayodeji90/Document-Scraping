#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Tunisia-focused PPTX scraper — Search-Engine-Driven (Africa Series)"""
...
TOPICS: List[str] = ['cours magistral', 'diaporama', 'présentation', 'matériel pédagogique', 'محاضرة', 'شرائح', 'informatique', 'ingénierie', 'physique', 'chimie', 'mathématiques', 'médecine']

SITE_DOMAINS: List[str] = ['site:ucar.rnu.tn', 'site:enit.rnu.tn', 'site:fst.rnu.tn', 'site:isi.rnu.tn', 'site:isg.rnu.tn', 'site:ensi.rnu.tn', 'site:enis.tn', 'site:supcom.tn']
...
def build_query_list() -> List[str]:
    queries = []
    for b in ['site:rnu.tn', 'site:tn', 'site:enit.rnu.tn', 'site:fst.rnu.tn']:
...
class TunisiaScraper:
    def __init__(self, out_dir="downloaded_ppts_tunisia", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 TN_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Tunisia Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Tunisia Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    TunisiaScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
