#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Ghana-focused PPTX scraper — Search-Engine-Driven (Africa Series)"""
...
TOPICS: List[str] = ['lecture slides', 'presentation', 'course materials', 'tutorial', 'computer science', 'engineering', 'physics', 'chemistry', 'biology', 'mathematics', 'medicine', 'economics', 'business', 'agriculture', 'law']

SITE_DOMAINS: List[str] = ['site:ug.edu.gh', 'site:knust.edu.gh', 'site:ucc.edu.gh', 'site:uew.edu.gh', 'site:uds.edu.gh', 'site:gimpa.edu.gh', 'site:ashesi.edu.gh', 'site:upsa.edu.gh']
...
def build_query_list() -> List[str]:
    queries = []
    for b in ['site:edu.gh', 'site:gh', 'site:ug.edu.gh', 'site:knust.edu.gh']:
...
class GhanaScraper:
    def __init__(self, out_dir="downloaded_ppts_ghana", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 GH_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Ghana Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Ghana Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    GhanaScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
