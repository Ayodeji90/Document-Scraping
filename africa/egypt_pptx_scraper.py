...
# -*- coding: utf-8 -*-
"""
Egypt-focused PPTX scraper — Search-Engine-Driven (Africa Series)
Discovers and downloads academic .pptx / .ppt files from Egyptian institutions (.eg).
"""
...
TOPICS: List[str] = [
    "محاضرة", "شرائح", "عرض تقديمي", "مادة دراسية", "ندوة",
    "lecture slides", "presentation", "course materials", "seminar", "tutorial",
    "علوم الحاسب", "ذكاء اصطناعي", "تعلم الآلة", "هندسة", "هندسة كهربائية", "هندسة مدنية",
    "فيزياء", "كيمياء", "أحياء", "رياضيات", "إحصاء", "طب", "تشريح", "صيدلة",
    "اقتصاد", "إدارة أعمال", "تسويق", "محاسبة", "تاريخ", "فلسفة", "علم نفس", "قانون",
    "computer science", "artificial intelligence", "machine learning",
    "engineering", "physics", "chemistry", "biology", "mathematics",
    "medicine", "economics", "agriculture", "petroleum engineering",
]

SITE_DOMAINS: List[str] = [
    "site:cu.edu.eg",          # Cairo University
    "site:alexu.edu.eg",       # Alexandria University
    "site:aun.edu.eg",         # Assiut University
    "site:mans.edu.eg",        # Mansoura University
    "site:asu.edu.eg",         # Ain Shams University
    "site:zu.edu.eg",          # Zagazig University
    "site:bu.edu.eg",          # Benha University
    "site:nile.edu.eg",        # Nile University
    "site:guc.edu.eg",         # German University in Cairo
    "site:auc.edu.eg",         # American University in Cairo
]

def build_query_list() -> List[str]:
    queries = []
    for b in ["site:edu.eg", "site:cu.edu.eg", "site:alexu.edu.eg", "site:eg"]:
        queries.append(f"filetype:pptx {b}"); queries.append(f"filetype:ppt {b}")
        queries.append(f"محاضرة شرائح filetype:pptx {b}")
    remaining = []
...
class EgyptScraper:
    def __init__(self, out_dir="downloaded_ppts_egypt", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 EG_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Egypt Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Egypt Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    EgyptScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
