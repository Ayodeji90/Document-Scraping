...
# -*- coding: utf-8 -*-
"""
South Korea-focused PPTX scraper — Search-Engine-Driven (Asia Series)
Discovers and downloads academic .pptx / .ppt files from South Korean institutions (.kr).

Strategies for 10,000+ files:
  1. Priority focus on SNU, KAIST, and POSTECH.
  2. Korean + English academic keywords (강의, 슬라이드, Slides).
...
# South Korean repositories often use DSpace or custom portal systems
...
# South Korean Academic Topics & Document Types (Korean + English)
TOPICS: List[str] = [
    # Document Types
    "강의", "슬라이드", "발표", "수업 자료", "세미나", "워크숍",
    "lecture slides", "presentation", "course materials", "seminar",
    
    # Specific Subjects
    "컴퓨터 공학", "인공지능", "머신러닝", "딥러닝",
    "전자 공학", "반도체", "나노 기술", "전기 공학",
    "물리학", "양자 역학", "열역학", "광학",
    "화학", "유기 화학", "생화학",
    "생물학", "유전학", "생명공학",
    "수학", "통계학", "미적분학", "대수학",
...
# Major South Korean Academic Domains
SITE_DOMAINS: List[str] = [
    "site:snu.ac.kr",        # Seoul National University
    "site:kaist.ac.kr",      # Korea Advanced Institute of Science and Technology
    "site:postech.ac.kr",    # Pohang University of Science and Technology
    "site:yonsei.ac.kr",     # Yonsei University
    "site:korea.ac.kr",      # Korea University
    "site:hanyang.ac.kr",    # Hanyang University (Engineering hub)
    "site:skku.edu",         # Sungkyunkwan University
...
def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for South Korea."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:snu.ac.kr", "site:kaist.ac.kr", "site:postech.ac.kr", "site:kr"]
    for b in broad:
        queries.append(f"filetype:pptx {b}")
        queries.append(f"filetype:ppt {b}")
        queries.append(f"강의 슬라이드 filetype:pptx {b}")
...
class SouthKoreaScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_south_korea",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 KR_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting South Korea Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="South Korea Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = SouthKoreaScraper(verify_ssl=not args.no_verify_ssl)
