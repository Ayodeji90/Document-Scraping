...
# -*- coding: utf-8 -*-
"""
Saudi Arabia-focused PPTX scraper — Search-Engine-Driven (Asia Series)
Discovers and downloads academic .pptx / .ppt files from Saudi institutions (.sa).

Strategies for 10,000+ files:
  1. Priority focus on KAUST, King Saud University, and KFUPM.
  2. Arabic + English academic keywords (محاضرة, شرائح, Slides).
...
# Saudi repositories often use DSpace or custom portal systems
...
# Saudi Academic Topics & Document Types (Arabic + English)
TOPICS: List[str] = [
    # Document Types
    "محاضرة", "شرائح", "عرض", "مواد دراسية", "ندوة", "دورة",
    "lecture slides", "presentation", "research talk", "course notes",
    
    # Specific Subjects
    "علوم الحاسب", "الذكاء الاصطناعي", "تعلم الآلة",
    "الهندسة", "الهندسة الميكانيكية", "الهندسة الكهربائية", "هندسة البترول",
    "الفيزياء", "ميكانيكا الكم", "الديناميكا الحرارية", "البصريات",
    "الكيمياء", "الكيمياء العضوية", "الكيمياء الحيوية",
    "الأحياء", "علم الوراثة", "التقنية الحيوية",
    "الرياضيات", "الإحصاء", "التفاضل والتكامل", "الجبر",
    "الطب", "التشريح", "علم وظائف الأعضاء", "الجراحة",
    "الاقتصاد", "الإدارة", "التسويق", "المالية",
    "التاريخ", "الفلسفة", "علم النفس", "علم الاجتماع", "القانون",
    "العمارة", "التصميم", "التخطيط الحضري"
]

# Major Saudi Academic Domains
SITE_DOMAINS: List[str] = [
    "site:ksu.edu.sa",       # King Saud University
    "site:kau.edu.sa",       # King Abdulaziz University
    "site:kaust.edu.sa",     # King Abdullah University of Science and Technology
    "site:kfupm.edu.sa",     # King Fahd University of Petroleum and Minerals
    "site:pnu.edu.sa",       # Princess Nourah bint Abdulrahman University
    "site:imamu.edu.sa",     # Imam Mohammad Ibn Saud Islamic University
    "site:uqu.edu.sa",       # Umm Al-Qura University
    "site:iau.edu.sa",       # Imam Abdulrahman Bin Faisal University
    "site:kku.edu.sa",       # King Khalid University
    "site:nu.edu.sa"         # Najran University
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for Saudi Arabia."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:kaust.edu.sa", "site:ksu.edu.sa", "site:kfupm.edu.sa", "site:sa"]
    for b in broad:
...
class SaudiArabiaScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_saudi_arabia",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 SA_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Saudi Arabia Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Saudi Arabia Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = SaudiArabiaScraper(verify_ssl=not args.no_verify_ssl)
