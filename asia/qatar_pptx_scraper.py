#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Qatar-focused PPTX scraper — Search-Engine-Driven (Asia Series)"""
...
TOPICS: List[str] = ['محاضرة', 'شرائح', 'عرض تقديمي', 'lecture slides', 'presentation', 'course materials', 'engineering', 'computer science', 'medicine', 'business', 'physics', 'chemistry', 'petroleum']

SITE_DOMAINS: List[str] = ['site:qu.edu.qa', 'site:hbku.edu.qa', 'site:cmu.edu.qa', 'site:qatar.tamu.edu', 'site:wcm-q.edu.qa', 'site:udst.edu.qa', 'site:gust.edu.kw']
...
def build_query_list() -> List[str]:
    queries = []
    for b in ['site:edu.qa', 'site:qa', 'site:qu.edu.qa', 'site:hbku.edu.qa']:
...
class QatarScraper:
    def __init__(self, out_dir="downloaded_ppts_qatar", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 QA_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Qatar Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Qatar Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    QatarScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
