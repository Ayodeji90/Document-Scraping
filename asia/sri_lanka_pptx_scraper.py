#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Sri Lanka-focused PPTX scraper — Search-Engine-Driven (Asia Series)"""
...
TOPICS: List[str] = ['lecture slides', 'presentation', 'course materials', 'tutorial', 'computer science', 'engineering', 'physics', 'chemistry', 'biology', 'mathematics', 'medicine', 'economics', 'business']

SITE_DOMAINS: List[str] = ['site:cmb.ac.lk', 'site:pdn.ac.lk', 'site:mrt.ac.lk', 'site:ruh.ac.lk', 'site:jfn.ac.lk', 'site:sjp.ac.lk', 'site:kln.ac.lk', 'site:sab.ac.lk']
...
def build_query_list() -> List[str]:
    queries = []
    for b in ['site:ac.lk', 'site:lk', 'site:cmb.ac.lk', 'site:mrt.ac.lk']:
...
class SriLankaScraper:
    def __init__(self, out_dir="downloaded_ppts_sri_lanka", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 LK_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Sri Lanka Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Sri Lanka Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    SriLankaScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
