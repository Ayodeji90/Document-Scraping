#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""China-focused PPTX scraper — Search-Engine-Driven (Asia Series)"""
...
TOPICS: List[str] = ['讲座', '课件', '演示文稿', '教学材料', '研讨会', '人工智能', '机器学习', '计算机科学', '工程', '物理', '化学', '生物', '数学', '医学', '经济学', '法学']

SITE_DOMAINS: List[str] = ['site:tsinghua.edu.cn', 'site:pku.edu.cn', 'site:fudan.edu.cn', 'site:sjtu.edu.cn', 'site:zju.edu.cn', 'site:ustc.edu.cn', 'site:hit.edu.cn', 'site:nju.edu.cn', 'site:whu.edu.cn', 'site:buaa.edu.cn']
...
def build_query_list() -> List[str]:
    queries = []
    for b in ['site:edu.cn', 'site:tsinghua.edu.cn', 'site:pku.edu.cn', 'site:cn']:
...
class ChinaScraper:
    def __init__(self, out_dir="downloaded_ppts_china", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 CN_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting China Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="China Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    ChinaScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
