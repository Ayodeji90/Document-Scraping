#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Philippines-focused PPTX scraper — Search-Engine-Driven (Asia Series)"""
...
TOPICS: List[str] = ['lecture slides', 'presentation', 'course materials', 'tutorial', 'computer science', 'artificial intelligence', 'machine learning', 'engineering', 'electrical engineering', 'civil engineering', 'physics', 'chemistry', 'biology', 'mathematics', 'medicine', 'nursing', 'pharmacy', 'economics', 'business', 'accounting', 'education', 'agriculture', 'environmental science', 'law', 'political science']

SITE_DOMAINS: List[str] = ['site:up.edu.ph', 'site:dlsu.edu.ph', 'site:ateneo.edu', 'site:ust.edu.ph', 'site:upd.edu.ph', 'site:admu.edu.ph', 'site:mapua.edu.ph', 'site:cpu.edu.ph', 'site:su.edu.ph', 'site:msu.edu.ph']

def build_query_list() -> List[str]:
    queries = []
    for b in ['site:edu.ph', 'site:up.edu.ph', 'site:dlsu.edu.ph', 'site:ph']:
...
class PhilippinesScraper:
    def __init__(self, out_dir="downloaded_ppts_philippines", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 PH_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Philippines Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Philippines Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    PhilippinesScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
