...
# -*- coding: utf-8 -*-
"""
Thailand-focused PPTX scraper — Search-Engine-Driven (Asia Series)
Discovers and downloads academic .pptx / .ppt files from Thai institutions (.th).

Strategies for 10,000+ files:
  1. Priority focus on Chulalongkorn, Mahidol, and KMUTT.
  2. Thai academic keywords (บทเรียน, สไลด์, การนำเสนอ).
...
# Thai repositories often use DSpace or custom portal systems
...
# Thai Academic Topics & Document Types (Thai + English)
TOPICS: List[str] = [
    # Document Types
    "บทเรียน", "สไลด์", "การนำเสนอ", "เอกสารประกอบการสอน", "สัมมนา",
    "lecture notes", "presentation", "course slides", "seminar",
    
    # Specific Subjects
    "วิทยาการคอมพิวเตอร์", "ปัญญาประดิษฐ์", "การเรียนรู้ของเครื่อง",
    "วิศวกรรม", "วิศวกรรมเครื่องกล", "วิศวกรรมไฟฟ้า", "วิศวกรรมโยธา",
    "ฟิสิกส์", "กลศาสตร์ควอนตัม", "อุณหพลศาสตร์", "ทัศนศาสตร์",
    "เคมี", "เคมีอินทรีย์", "ชีวเคมี",
    "ชีววิทยา", "พันธุศาสตร์", "เทคโนโลยีชีวภาพ",
    "คณิตศาสตร์", "สถิติ", "แคลคูลัส", "พีชคณิต",
    "การแพทย์", "กายวิภาคศาสตร์", "สรีรวิทยา", "ศัลยกรรม",
    "เศรษฐศาสตร์", "การจัดการ", "การตลาด", "การเงิน",
    "ประวัติศาสตร์", "ปรัชญา", "จิตวิทยา", "สังคมวิทยา", "กฎหมาย",
    "สถาปัตยกรรม", "การออกแบบ", "การวางผังเมือง"
]

# Major Thai Academic Domains
SITE_DOMAINS: List[str] = [
    "site:chula.ac.th",      # Chulalongkorn University
    "site:mahidol.ac.th",    # Mahidol University
    "site:kaset.ac.th",      # Kasetsart University
    "site:tu.ac.th",         # Thammasat University
    "site:cmu.ac.th",        # Chiang Mai University
    "site:kku.ac.th",        # Khon Kaen University
    "site:psu.ac.th",        # Prince of Songkla University
    "site:kmutt.ac.th",      # KMUTT (Tech focus)
    "site:kmitl.ac.th",      # KMITL (Tech focus)
    "site:swu.ac.th"         # Srinakharinwirot University
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for Thailand."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:chula.ac.th", "site:mahidol.ac.th", "site:kmutt.ac.th", "site:th"]
    for b in broad:
        queries.append(f"filetype:pptx {b}")
        queries.append(f"filetype:ppt {b}")
        queries.append(f"บทเรียน สไลด์ filetype:pptx {b}")
...
class ThailandScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_thailand",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 TH_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Thailand Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Thailand Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = ThailandScraper(verify_ssl=not args.no_verify_ssl)
