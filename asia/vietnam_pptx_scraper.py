...
# -*- coding: utf-8 -*-
"""
Vietnam-focused PPTX scraper — Search-Engine-Driven (Asia Series)
Discovers and downloads academic .pptx / .ppt files from Vietnamese institutions (.vn).

Strategies for 10,000+ files:
  1. Priority focus on HUST, VNU, and HCMUT.
  2. Vietnamese + English academic keywords (Bài giảng, Slide, Thuyết trình).
...
# Vietnamese repositories often use DSpace or custom portal systems
BITSTREAM_RE = re.compile(
    r"/(?:bitstream(?:/handle)?/[\d./]+|retrieve/\d+|download/\d+|file|attachment|get|tai-lieu)/[^\"\'\s>{}\[\]\\]+\.pptx?",
    re.IGNORECASE,
)
...
# Vietnamese Academic Topics & Document Types (Vietnamese + English)
TOPICS: List[str] = [
    # Document Types
    "bài giảng", "slide bài giảng", "thuyết trình", "tài liệu học tập", "hội thảo", "giáo trình",
    "lecture slides", "presentation", "course materials", "seminar",
    
    # Specific Subjects
    "khoa học máy tính", "trí tuệ nhân tạo", "học máy",
    "kỹ thuật", "kỹ thuật cơ khí", "kỹ thuật điện", "kỹ thuật xây dựng",
    "vật lý", "cơ học lượng tử", "nhiệt động lực học", "quang học",
    "hóa học", "hóa hữu cơ", "biến đổi hóa học",
    "sinh học", "di truyền học", "công nghệ sinh học",
    "toán học", "thống kê", "giải tích", "đại số",
    "y học", "giải phẫu", "sinh lý học", "phẫu thuật",
    "kinh tế", "quản trị", "marketing", "tài chính",
    "lịch sử", "triết học", "tâm lý học", "xã hội học", "luật",
    "kiến trúc", "thiết kế", "quy hoạch đô thị"
]

# Major Vietnamese Academic Domains
SITE_DOMAINS: List[str] = [
    "site:vnu.edu.vn",       # Vietnam National University
    "site:hust.edu.vn",      # Hanoi University of Science and Technology
    "site:hcmut.edu.vn",     # Ho Chi Minh City University of Technology
    "site:ueh.edu.vn",       # University of Economics Ho Chi Minh City
    "site:neu.edu.vn",       # National Economics University
    "site:duytan.edu.vn",    # Duy Tan University
    "site:tonducthang.edu.vn", # Ton Duc Thang University
    "site:fpt.edu.vn",       # FPT University
    "site:cantho.edu.vn",    # Can Tho University
    "site:ptit.edu.vn"       # Posts and Telecommunications Institute of Technology
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for Vietnam."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:hust.edu.vn", "site:vnu.edu.vn", "site:hcmut.edu.vn", "site:vn"]
    for b in broad:
        queries.append(f"filetype:pptx {b}")
        queries.append(f"filetype:ppt {b}")
        queries.append(f"bài giảng slide filetype:pptx {b}")
...
class VietnamScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_vietnam",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 VN_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Vietnam Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Vietnam Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = VietnamScraper(verify_ssl=not args.no_verify_ssl)
