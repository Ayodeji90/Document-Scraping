#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Kazakhstan-focused PPTX scraper — Search-Engine-Driven (Asia Series)"""
...
TOPICS: List[str] = ['лекция', 'презентация', 'учебные материалы', 'семинар', 'дәріс', 'информатика', 'инженерия', 'физика', 'химия', 'биология', 'математика', 'медицина', 'экономика']

SITE_DOMAINS: List[str] = ['site:kaznu.kz', 'site:nu.edu.kz', 'site:satbayev.university', 'site:enu.kz', 'site:kbtu.kz', 'site:aitu.edu.kz', 'site:iitu.edu.kz', 'site:kimep.kz']
...
def build_query_list() -> List[str]:
    queries = []
    for b in ['site:kz', 'site:edu.kz', 'site:kaznu.kz', 'site:nu.edu.kz']:
...
class KazakhstanScraper:
    def __init__(self, out_dir="downloaded_ppts_kazakhstan", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 KZ_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Kazakhstan Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Kazakhstan Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    KazakhstanScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
