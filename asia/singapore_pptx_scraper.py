...
# -*- coding: utf-8 -*-
"""
Singapore-focused PPTX scraper — Search-Engine-Driven (Asia Series)
Discovers and downloads academic .pptx / .ppt files from Singaporean institutions (.sg).

Strategies for 10,000+ files:
  1. Priority focus on NUS, NTU, and SUTD.
  2. English academic keywords (Lecture slides, Technical presentation).
...
# Singaporean repositories often use DSpace or custom portal systems
...
# Singaporean Academic Topics & Document Types (English)
TOPICS: List[str] = [
    # Document Types
    ...
    "history", "philosophy", "sociology", "psychology",
    "architecture", "sustainable design", "urban planning"
]

# Major Singaporean Academic Domains
SITE_DOMAINS: List[str] = [
    "site:nus.edu.sg",       # National University of Singapore
    "site:ntu.edu.sg",       # Nanyang Technological University
    "site:smu.edu.sg",       # Singapore Management University
    "site:sutd.edu.sg",      # Singapore University of Technology and Design
    "site:singaporetech.edu.sg", # Singapore Institute of Technology
    "site:suss.edu.sg",      # Singapore University of Social Sciences
    "site:nie.edu.sg",       # National Institute of Education
    "site:dukenus.edu.sg",   # Duke-NUS Medical School
    "site:a-star.edu.sg"     # A*STAR (Agency for Science, Technology and Research)
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for Singapore."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:nus.edu.sg", "site:ntu.edu.sg", "site:sutd.edu.sg", "site:sg"]
    for b in broad:
...
class SingaporeScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_singapore",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 SG_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Singapore Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Singapore Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = SingaporeScraper(verify_ssl=not args.no_verify_ssl)
