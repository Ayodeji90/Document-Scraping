#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""UAE-focused PPTX scraper — Search-Engine-Driven (Asia Series)"""
...
TOPICS: List[str] = ['محاضرة', 'شرائح', 'عرض تقديمي', 'مادة دراسية', 'ندوة', 'lecture slides', 'presentation', 'course materials', 'seminar', 'computer science', 'artificial intelligence', 'machine learning', 'engineering', 'physics', 'chemistry', 'biology', 'mathematics', 'medicine', 'business', 'finance', 'economics', 'law', 'petroleum engineering', 'aerospace engineering']

SITE_DOMAINS: List[str] = ['site:khalifa.edu', 'site:uaeu.ac.ae', 'site:aus.edu', 'site:adu.ac.ae', 'site:zu.ac.ae', 'site:ajman.ac.ae', 'site:ud.ac.ae', 'site:hbku.edu.qa', 'site:ku.ac.ae', 'site:masdar.ac.ae']

def build_query_list() -> List[str]:
    queries = []
    for b in ['site:ac.ae', 'site:edu.ae', 'site:khalifa.edu', 'site:ae']:
...
class UAEScraper:
    def __init__(self, out_dir="downloaded_ppts_uae", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 AE_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting UAE Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="UAE Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    UAEScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
