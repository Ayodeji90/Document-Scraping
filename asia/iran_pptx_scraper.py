#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Iran-focused PPTX scraper — Search-Engine-Driven (Asia Series)"""
...
TOPICS: List[str] = ['اسلاید', 'ارائه', 'جزوه', 'سمینار', 'هوش مصنوعی', 'مهندسی', 'فیزیک', 'شیمی', 'ریاضی', 'پزشکی', 'اقتصاد', 'حقوق']

SITE_DOMAINS: List[str] = ['site:ut.ac.ir', 'site:sharif.edu', 'site:aut.ac.ir', 'site:iust.ac.ir', 'site:modares.ac.ir', 'site:sbu.ac.ir', 'site:kntu.ac.ir', 'site:guilan.ac.ir', 'site:shirazu.ac.ir', 'site:um.ac.ir']
...
def build_query_list() -> List[str]:
    queries = []
    for b in ['site:ac.ir', 'site:ut.ac.ir', 'site:sharif.edu', 'site:ir']:
...
class IranScraper:
    def __init__(self, out_dir="downloaded_ppts_iran", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 IR_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Iran Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Iran Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    IranScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
