...
# -*- coding: utf-8 -*-
"""
Malaysia-focused PPTX scraper — Search-Engine-Driven (Asia Series)
Discovers and downloads academic .pptx / .ppt files from Malaysian institutions (.my).

Strategies for 10,000+ files:
  1. Priority focus on UM, UTM, and USM.
  2. English + Malay academic keywords (Lecture slides, Slaid, Kuliah).
...
# Malaysian repositories often use DSpace or custom portal systems
...
# Major Malaysian Academic Domains
SITE_DOMAINS: List[str] = [
    "site:um.edu.my",        # University of Malaya
    "site:utm.my",           # Universiti Teknologi Malaysia
    "site:upm.edu.my",       # Universiti Putra Malaysia
    "site:ukm.my",           # Universiti Kebangsaan Malaysia
    "site:usm.my",           # Universiti Sains Malaysia
    "site:uitm.edu.my",      # Universiti Teknologi MARA
    "site:iium.edu.my",      # International Islamic University Malaysia
    "site:taylors.edu.my",   # Taylor's University
    "site:monash.edu.my",    # Monash University Malaysia
    "site:mmu.edu.my"        # Multimedia University
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for Malaysia."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:um.edu.my", "site:utm.my", "site:usm.my", "site:my"]
    for b in broad:
...
class MalaysiaScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_malaysia",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 MY_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Malaysia Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Malaysia Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = MalaysiaScraper(verify_ssl=not args.no_verify_ssl)
