...
# -*- coding: utf-8 -*-
"""
Israel-focused PPTX scraper — Search-Engine-Driven (Asia Series)
Discovers and downloads academic .pptx / .ppt files from Israeli institutions (.il).

Strategies for 10,000+ files:
  1. Priority focus on Technion, Hebrew University, and Tel Aviv University.
  2. Hebrew + English academic keywords (מצגת, הרצאה, Slides).
...
# Israeli repositories often use DSpace or custom portal systems
...
# Israeli Academic Topics & Document Types (Hebrew + English)
TOPICS: List[str] = [
    # Document Types
    "מצגת", "הרצאה", "שקפים", "סיכום", "קורס", "סמינריון",
    "lecture slides", "presentation", "research presentation", "course notes",
    
    # Specific Subjects
    "מדעי המחשב", "בינה מלאכותית", "למידת מכונה",
    "הנדסה", "הנדסת חשמל", "הנדסת מכונות", "הנדסה אזרחית",
    "פיזיקה", "מכניקת הקוונטים", "תרמודינמיקה", "אופטיקה",
    "כימיה", "כימיה אורגנית", "ביוכימיה",
    "ביולוגיה", "גנטיקה", "ביוטכנולוגיה",
    "מתמטיקה", "סטטיסטיקה", "אלגברה", "אנליזה",
    "רפואה", "אנטומיה", "פיזיולוגיה", "כירורגיה",
    "כלכלה", "ניהול", "שיווק", "מימון",
    "היסטוריה", "פילוסופיה", "פסיכולוגיה", "סוציולוגיה", "משפטים",
    "ארכיטקטורה", "עיצוב", "תכנון עירוני"
]

# Major Israeli Academic Domains
SITE_DOMAINS: List[str] = [
    "site:technion.ac.il",   # Technion - Israel Institute of Technology
    "site:huji.ac.il",       # Hebrew University of Jerusalem
    "site:tau.ac.il",        # Tel Aviv University
    "site:weizmann.ac.il",   # Weizmann Institute of Science
    "site:biu.ac.il",        # Bar-Ilan University
    "site:bgu.ac.il",        # Ben-Gurion University of the Negev
    "site:haifa.ac.il",      # University of Haifa
    "site:idc.ac.il",        # Reichman University (IDC Herzliya)
    "site:ariel.ac.il",      # Ariel University
    "site:openu.ac.il"       # Open University of Israel
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for Israel."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:technion.ac.il", "site:huji.ac.il", "site:tau.ac.il", "site:il"]
    for b in broad:
        queries.append(f"filetype:pptx {b}")
        queries.append(f"filetype:ppt {b}")
        queries.append(f"מצגת שקפים filetype:pptx {b}")
...
class IsraelScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_israel",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 IL_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Israel Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Israel Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = IsraelScraper(verify_ssl=not args.no_verify_ssl)
