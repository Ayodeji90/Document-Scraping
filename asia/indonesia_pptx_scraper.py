#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Indonesia-focused PPTX scraper — Search-Engine-Driven (Asia Series)"""
...
try:
    ...
    raise SystemExit("ddgs not installed.")
...
TOPICS: List[str] = [
    "materi kuliah", "slide presentasi", "bahan ajar", "perkuliahan", "seminar",
    "lecture slides", "presentation", "course materials", "tutorial",
    "ilmu komputer", "kecerdasan buatan", "pembelajaran mesin",
    "teknik", "teknik mesin", "teknik elektro", "teknik sipil",
    "fisika", "kimia", "biologi", "matematika", "statistika",
    "kedokteran", "farmasi", "kesehatan masyarakat",
    "ekonomi", "manajemen", "akuntansi", "hukum", "pertanian",
    "computer science", "artificial intelligence", "engineering",
    "physics", "chemistry", "biology", "mathematics", "medicine", "economics",
]

SITE_DOMAINS: List[str] = [
    "site:ui.ac.id",           # Universitas Indonesia
    "site:ugm.ac.id",          # Universitas Gadjah Mada
    "site:itb.ac.id",          # Institut Teknologi Bandung
    "site:its.ac.id",          # Institut Teknologi Sepuluh Nopember
    "site:unair.ac.id",        # Universitas Airlangga
    "site:undip.ac.id",        # Universitas Diponegoro
    "site:ipb.ac.id",          # IPB University
    "site:unpad.ac.id",        # Universitas Padjadjaran
    "site:ub.ac.id",           # Universitas Brawijaya
    "site:uns.ac.id",          # Universitas Sebelas Maret
]

def build_query_list() -> List[str]:
    queries = []
    for b in ["site:ac.id", "site:ui.ac.id", "site:ugm.ac.id", "site:itb.ac.id"]:
        queries.append(f"filetype:pptx {b}"); queries.append(f"filetype:ppt {b}")
        queries.append(f"materi kuliah slide filetype:pptx {b}")
    remaining = []
    for domain in SITE_DOMAINS:
        for topic in TOPICS:
            remaining.append(f'"{topic}" filetype:pptx {domain}')
            if random.random() < 0.2: remaining.append(f'"{topic}" filetype:ppt {domain}')
    random.shuffle(remaining)
    return queries + remaining

class IndonesiaScraper:
    def __init__(self, out_dir="downloaded_ppts_indonesia", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
        ...
        if not verify_ssl: warnings.filterwarnings("ignore", category=urllib3.exceptions.InsecureRequestWarning)
    ...
    def _preload_seen_from_disk(self):
        ...
        if self._seen_tags: logger.info("Resuming: Loaded %d seen tags.", len(self._seen_tags))
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 ID_Scraper/1.0 (Academic Research)"}); return s
    ...
    def _safe_filename(self, url):
        ...
        if not name.lower().endswith((".pptx",".ppt")): return None, None
        return tag, f"{tag}_{re.sub(r'[^\\w.\\-]','_',name)}"
    ...
    def _download(self, url):
        ...
        try:
            ...
            if dest.stat().st_size < 2097152: dest.unlink(missing_ok=True); return None
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Indonesia Scale-Up: Target=%d, Queries=%d", target, len(queries))
            ...
            for url in self._search_ddgs(query):
                ...
                if PRESENTATION_RE.search(url): to_download.append(url)
                    ...
                    for iu in self._extract_from_page(url):
                        ...
                        if iu not in self._seen_urls and itag not in self._seen_tags: to_download.append(iu); self._seen_urls.add(iu)
...
def main():
    parser = argparse.ArgumentParser(description="Indonesia Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    IndonesiaScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
