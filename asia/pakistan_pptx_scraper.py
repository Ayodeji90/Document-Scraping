#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Pakistan-focused PPTX scraper — Search-Engine-Driven (Asia Series)"""
...
TOPICS: List[str] = ['lecture slides', 'presentation', 'course materials', 'computer science', 'artificial intelligence', 'engineering', 'electrical engineering', 'civil engineering', 'mechanical engineering', 'physics', 'chemistry', 'biology', 'mathematics', 'medicine', 'pharmacy', 'economics', 'business', 'accounting', 'agriculture', 'law', 'education', 'محاضرة', 'سلائیڈز']

SITE_DOMAINS: List[str] = ['site:nust.edu.pk', 'site:lums.edu.pk', 'site:uet.edu.pk', 'site:qu.edu.pk', 'site:uop.edu.pk', 'site:pu.edu.pk', 'site:giki.edu.pk', 'site:comsats.edu.pk', 'site:iba.edu.pk', 'site:fast.edu.pk']

def build_query_list() -> List[str]:
    queries = []
    for b in ['site:edu.pk', 'site:nust.edu.pk', 'site:lums.edu.pk', 'site:pk']:
...
class PakistanScraper:
    def __init__(self, out_dir="downloaded_ppts_pakistan", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 PK_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Pakistan Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Pakistan Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    PakistanScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
