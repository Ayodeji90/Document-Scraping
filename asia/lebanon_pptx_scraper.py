#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Lebanon-focused PPTX scraper — Search-Engine-Driven (Asia Series)"""
...
TOPICS: List[str] = ['محاضرة', 'شرائح', 'عرض تقديمي', 'cours', 'présentation', 'diaporama', 'lecture slides', 'engineering', 'computer science', 'medicine', 'business', 'physics', 'chemistry']

SITE_DOMAINS: List[str] = ['site:aub.edu.lb', 'site:lau.edu.lb', 'site:usj.edu.lb', 'site:ul.edu.lb', 'site:ndu.edu.lb', 'site:bau.edu.lb', 'site:balamand.edu.lb', 'site:auce.edu.lb']
...
def build_query_list() -> List[str]:
    queries = []
    for b in ['site:edu.lb', 'site:lb', 'site:aub.edu.lb', 'site:lau.edu.lb']:
...
class LebanonScraper:
    def __init__(self, out_dir="downloaded_ppts_lebanon", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 LB_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Lebanon Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Lebanon Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    LebanonScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
