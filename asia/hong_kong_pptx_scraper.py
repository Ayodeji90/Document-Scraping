...
# -*- coding: utf-8 -*-
"""
Hong Kong-focused PPTX scraper — Search-Engine-Driven (Asia Series)
Discovers and downloads academic .pptx / .ppt files from Hong Kong institutions (.hk).

Strategies for 10,000+ files:
  1. Priority focus on HKU, HKUST, and CUHK.
...
# Hong Kong repositories often use DSpace or custom portal systems
...
# Hong Kong Academic Topics & Document Types (English)
TOPICS: List[str] = [
    # Document Types
    ...
    "finance", "investment banking", "economics", "accounting",
...
# Major Hong Kong Academic Domains
SITE_DOMAINS: List[str] = [
    "site:hku.hk",           # University of Hong Kong
    "site:ust.hk",           # Hong Kong University of Science and Technology
    "site:cuhk.edu.hk",      # Chinese University of Hong Kong
    "site:polyu.edu.hk",     # Hong Kong Polytechnic University
    "site:cityu.edu.hk",     # City University of Hong Kong
    "site:hkbu.edu.hk",      # Hong Kong Baptist University
    "site:ln.edu.hk",        # Lingnan University
    "site:eduhk.hk",         # Education University of Hong Kong
    "site:vtc.edu.hk",       # Vocational Training Council
    "site:hkapa.edu"         # Hong Kong Academy for Performing Arts
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for Hong Kong."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:hku.hk", "site:ust.hk", "site:cuhk.edu.hk", "site:hk"]
    for b in broad:
...
class HongKongScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_hong_kong",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 HK_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Hong Kong Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Hong Kong Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = HongKongScraper(verify_ssl=not args.no_verify_ssl)
