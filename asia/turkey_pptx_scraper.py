...
# -*- coding: utf-8 -*-
"""
Turkey-focused PPTX scraper — Search-Engine-Driven (Asia/Europe Series)
Discovers and downloads academic .pptx / .ppt files from Turkish institutions (.tr).

Strategies for 10,000+ files:
  1. Priority focus on METU, ITU, and Boğaziçi University.
  2. Turkish academic keywords (Ders notları, Sunum, Slaytlar).
...
# Turkish repositories often use DSpace or custom portal systems
...
# Turkish Academic Topics & Document Types (Turkish + English)
TOPICS: List[str] = [
    # Document Types
    "ders notları", "sunum", "slaytlar", "ders sunumu", "seminer",
    "lecture notes", "presentation", "course slides", "seminar",
    
    # Specific Subjects
    "bilgisayar mühendisliği", "yapay zeka", "makine öğrenmesi",
    "mühendislik", "makine mühendisliği", "elektrik elektronik", "inşaat mühendisliği",
    "fizik", "kuantum mekaniği", "termodinamik", "optik",
    "kimya", "organik kimya", "biyokimya",
    "biyoloji", "genetik", "biyoteknoloji",
    "matematik", "istatistik", "calculus", "cebir",
    "tıp", "anatomi", "fizyoloji", "cerrahi",
    "ekonomi", "işletme", "pazarlama", "finans",
    "tarih", "felsefe", "psikoloji", "sosyoloji", "hukuk",
    "mimarlık", "tasarım", "şehir planlama"
]

# Major Turkish Academic Domains
SITE_DOMAINS: List[str] = [
    "site:metu.edu.tr",      # Middle East Technical University
    "site:itu.edu.tr",       # Istanbul Technical University
    "site:boun.edu.tr",      # Boğaziçi University
    "site:bilkent.edu.tr",   # Bilkent University
    "site:hacettepe.edu.tr", # Hacettepe University
    "site:istanbul.edu.tr",  # Istanbul University
    "site:ankara.edu.tr",    # Ankara University
    "site:sabanciuniv.edu",  # Sabancı University
    "site:ku.edu.tr",        # Koç University
    "site:yok.gov.tr"        # Higher Education Council (National Portal)
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for Turkey."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:metu.edu.tr", "site:itu.edu.tr", "site:boun.edu.tr", "site:tr"]
    for b in broad:
        queries.append(f"filetype:pptx {b}")
        queries.append(f"filetype:ppt {b}")
        queries.append(f"ders notları slaytlar filetype:pptx {b}")
...
class TurkeyScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_turkey",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 TR_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Turkey Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Turkey Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = TurkeyScraper(verify_ssl=not args.no_verify_ssl)
