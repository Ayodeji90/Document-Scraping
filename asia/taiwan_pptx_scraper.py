...
# -*- coding: utf-8 -*-
"""
Taiwan-focused PPTX scraper — Search-Engine-Driven (Asia Series)
Discovers and downloads academic .pptx / .ppt files from Taiwanese institutions (.tw).

Strategies for 10,000+ files:
  1. Priority focus on NTU, Tsing Hua (NTHU), and Chiao Tung (NYCU).
  2. Traditional Chinese + English academic keywords (投影片, 課程, Slides).
...
# Taiwanese repositories often use DSpace or custom portal systems
...
# Taiwanese Academic Topics & Document Types (Traditional Chinese + English)
TOPICS: List[str] = [
    # Document Types
    "課程", "投影片", "簡報", "講義", "研討會", "演講",
...
# Major Taiwanese Academic Domains
SITE_DOMAINS: List[str] = [
    "site:ntu.edu.tw",       # National Taiwan University
    "site:nthu.edu.tw",      # National Tsing Hua University
    "site:nycu.edu.tw",      # National Yang Ming Chiao Tung University
    "site:ncku.edu.tw",      # National Cheng Kung University
    "site:ntust.edu.tw",     # National Taiwan University of Science and Technology
    "site:nsysu.edu.tw",     # National Sun Yat-sen University
    "site:ccu.edu.tw",       # National Chung Cheng University
    "site:ntnu.edu.tw",      # National Taiwan Normal University
    "site:nchu.edu.tw",      # National Chung Hsing University
    "site:sinica.edu.tw"     # Academia Sinica (Top Research Institute)
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for Taiwan."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:ntu.edu.tw", "site:nthu.edu.tw", "site:nycu.edu.tw", "site:tw"]
    for b in broad:
        queries.append(f"filetype:pptx {b}")
        queries.append(f"filetype:ppt {b}")
        queries.append(f"課程 投影片 filetype:pptx {b}")
...
class TaiwanScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_taiwan",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 TW_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Taiwan Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Taiwan Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = TaiwanScraper(verify_ssl=not args.no_verify_ssl)
