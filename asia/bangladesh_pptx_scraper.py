#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Bangladesh-focused PPTX scraper — Search-Engine-Driven (Asia Series)"""
...
TOPICS: List[str] = ['lecture slides', 'presentation', 'course materials', 'computer science', 'artificial intelligence', 'engineering', 'electrical engineering', 'civil engineering', 'physics', 'chemistry', 'biology', 'mathematics', 'medicine', 'pharmacy', 'economics', 'business', 'accounting', 'agriculture', 'education', 'বক্তৃতা', 'উপস্থাপনা']

SITE_DOMAINS: List[str] = ['site:buet.ac.bd', 'site:du.ac.bd', 'site:ru.ac.bd', 'site:cuet.ac.bd', 'site:kuet.ac.bd', 'site:sust.edu', 'site:nsu.edu.bd', 'site:bracu.ac.bd', 'site:aiub.edu', 'site:iub.edu.bd']

def build_query_list() -> List[str]:
    queries = []
    for b in ['site:ac.bd', 'site:edu.bd', 'site:buet.ac.bd', 'site:bd']:
...
class BangladeshScraper:
    def __init__(self, out_dir="downloaded_ppts_bangladesh", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 BD_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Bangladesh Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Bangladesh Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    BangladeshScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
