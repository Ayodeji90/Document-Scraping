#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Jordan-focused PPTX scraper — Search-Engine-Driven (Asia Series)"""
...
TOPICS: List[str] = ['محاضرة', 'شرائح', 'عرض تقديمي', 'مادة دراسية', 'ندوة', 'علوم حاسوب', 'هندسة', 'فيزياء', 'كيمياء', 'أحياء', 'رياضيات', 'طب', 'اقتصاد', 'قانون']

SITE_DOMAINS: List[str] = ['site:just.edu.jo', 'site:ju.edu.jo', 'site:yu.edu.jo', 'site:hu.edu.jo', 'site:bau.edu.jo', 'site:aau.edu.jo', 'site:psut.edu.jo', 'site:gju.edu.jo']
...
def build_query_list() -> List[str]:
    queries = []
    for b in ['site:edu.jo', 'site:jo', 'site:just.edu.jo', 'site:ju.edu.jo']:
...
class JordanScraper:
    def __init__(self, out_dir="downloaded_ppts_jordan", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 JO_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Jordan Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Jordan Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    JordanScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
