...
#   scraper   - ckan_pptx_scraper.py, harvesting + downloading to ckan_downloaded/
#               (isolated from mega/commoncrawl/wayback's shared joint_downloaded/)
#   delivery  - deliver_ckan_to_gdrive.py --watch, validating + packaging into a
#               local staging folder as BATCH_03 (independent numbering from the
#               rest of the pipeline's BATCH_02) — kept local so the Python
#               process never depends on a live FUSE mount for writes
...
STAGING_DIR="$ROOT/gdrive_staging_ckan"
...
tmux new-window -t "$SESSION" -n delivery \
  "cd $ROOT && source venv/bin/activate && python3 -u deliver_ckan_to_gdrive.py --gdrive-path $STAGING_DIR --watch --interval 120; bash"
