...
# -*- coding: utf-8 -*-
"""
Mexico-focused PPTX scraper — Search-Engine-Driven (North America Series)
Discovers and downloads academic .pptx / .ppt files from Mexican institutions (.mx).

Strategies for 10,000+ files:
  1. Priority focus on UNAM, IPN, and Tecnológico de Monterrey (ITESM).
  2. Mexican Spanish academic keywords (Clase, Slides, Materia).
...
# Mexican repositories often use DSpace (RI-UNAM, etc.)
...
# Mexican Academic Topics & Document Types (Spanish + English)
TOPICS: List[str] = [
    # Document Types
    "clase", "slides", "presentación", "apuntes", "materia", "seminario",
    ...
    "historia", "filosofía", "psicologia", "sociologia", "derecho",
    "arquitectura", "diseño", "urbanismo"
]

# Major Mexican Academic Domains
SITE_DOMAINS: List[str] = [
    "site:unam.mx",          # National Autonomous University of Mexico
    "site:ipn.mx",           # National Polytechnic Institute
    "site:tec.mx",           # Tecnológico de Monterrey (ITESM)
    "site:uam.mx",           # Autonomous Metropolitan University
    "site:udem.edu.mx",      # University of Monterrey
    "site:unadmexico.mx",    # Open and Distance University of Mexico
    "site:buap.mx",          # Benemérita Universidad Autónoma de Puebla
    "site:uanl.mx",          # Autonomous University of Nuevo León
    "site:udg.mx",           # University of Guadalajara
    "site:conacyt.mx"        # CONACYT (National Research Council)
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for Mexico."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:unam.mx", "site:ipn.mx", "site:tec.mx", "site:mx"]
    for b in broad:
...
class MexicoScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_mexico",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 MX_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Mexico Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Mexico Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = MexicoScraper(verify_ssl=not args.no_verify_ssl)
