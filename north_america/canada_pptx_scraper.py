...
# -*- coding: utf-8 -*-
"""
Canada-focused PPTX scraper — Search-Engine-Driven (North America Series)
Discovers and downloads academic .pptx / .ppt files from Canadian institutions (.ca).

Strategies for 10,000+ files:
  1. Priority focus on UofT, UBC, McGill, and Waterloo.
  2. Bilingual query matrix (English + French).
  3. Stealth Mode: 5.0s delay and 300-result depth to avoid bans.
  4. Deep Discovery: Follows HTML pages to find embedded bitstream links in repositories (TSpace, cIRcle, etc.).
"""
...
# Canadian repositories often use DSpace (TSpace, cIRcle, etc.)
...
# Canadian Academic Topics & Document Types (English + French)
TOPICS: List[str] = [
    # Document Types (English)
    "lecture slides", "presentation", "research presentation", "course notes",
    "seminar presentation", "workshop materials",
    
    # Document Types (French)
    "cours", "diaporama", "présentation", "notes de cours",
    
    # Specific Subjects
    "computer science", "artificial intelligence", "machine learning",
    "engineering", "robotics", "nanotechnology",
...
# Major Canadian Academic Domains
SITE_DOMAINS: List[str] = [
    "site:utoronto.ca",      # University of Toronto
    "site:ubc.ca",           # University of British Columbia
    "site:mcgill.ca",        # McGill
    "site:uwaterloo.ca",     # Waterloo (Tech Hub)
    "site:ualberta.ca",      # Alberta
    "site:umontreal.ca",     # Montreal (French Hub)
    "site:uottawa.ca",       # Ottawa (Bilingual Hub)
    "site:queensu.ca",       # Queen's
    "site:westernu.ca",      # Western
    "site:ucalgary.ca",      # Calgary
    "site:sfu.ca",           # Simon Fraser
    "site:ulaval.ca",        # Laval (French Hub)
    "site:nrc-cnrc.gc.ca"    # National Research Council Canada
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for Canada."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:utoronto.ca", "site:ubc.ca", "site:uwaterloo.ca", "site:ca"]
    for b in broad:
...
class CanadaScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_canada",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 CA_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Canada Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Canada Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = CanadaScraper(verify_ssl=not args.no_verify_ssl)
