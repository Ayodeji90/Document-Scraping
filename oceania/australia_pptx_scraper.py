...
# -*- coding: utf-8 -*-
"""
Australia-focused PPTX scraper — Search-Engine-Driven (Oceania Series)
Discovers and downloads academic .pptx / .ppt files from Australian institutions (.au).

Strategies for 10,000+ files:
  1. Priority focus on Group of Eight (Melbourne, Sydney, ANU, etc.).
...
# Australian repositories often use DSpace or custom portal systems
...
# Australian Academic Topics & Document Types (English)
TOPICS: List[str] = [
    # Document Types
    "lecture slides", "presentation", "course notes", "seminar slides",
    "workshop presentation", "research presentation", "tutorial slides",
    ...
    "architecture", "urban design", "law"
]

# Major Australian Academic Domains
SITE_DOMAINS: List[str] = [
    "site:unimelb.edu.au",    # University of Melbourne
    "site:sydney.edu.au",     # University of Sydney
    "site:anu.edu.au",        # Australian National University
    "site:uq.edu.au",         # University of Queensland
    "site:unsw.edu.au",       # UNSW Sydney
    "site:monash.edu",        # Monash University
    "site:uwa.edu.au",        # University of Western Australia
    "site:adelaide.edu.au",   # University of Adelaide
    "site:uts.edu.au",        # University of Technology Sydney
    "site:rmit.edu.au"        # RMIT University
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for Australia."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:unimelb.edu.au", "site:sydney.edu.au", "site:anu.edu.au", "site:au"]
    for b in broad:
...
class AustraliaScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_australia",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 AU_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Australia Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Australia Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = AustraliaScraper(verify_ssl=not args.no_verify_ssl)
