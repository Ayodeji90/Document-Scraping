...
# -*- coding: utf-8 -*-
"""
New Zealand-focused PPTX scraper — Search-Engine-Driven (Oceania Series)
Discovers and downloads academic .pptx / .ppt files from New Zealand institutions (.nz).

Strategies for 10,000+ files:
  1. Priority focus on Auckland, Otago, and Canterbury.
  2. English academic keywords (Lecture slides, Course material).
...
# New Zealand repositories often use DSpace or custom portal systems
...
# New Zealand Academic Topics & Document Types (English)
TOPICS: List[str] = [
    # Document Types
    "lecture slides", "presentation", "course notes", "seminar slides",
    "technical presentation", "research talk", "tutorial material",
    
    # Specific Subjects
    "computer science", "artificial intelligence", "machine learning",
    "marine biology", "agriculture", "environmental science",
    ...
    "history", "philosophy", "sociology", "psychology", "law",
    "architecture", "urban design", "education"
]

# Major New Zealand Academic Domains
SITE_DOMAINS: List[str] = [
    "site:auckland.ac.nz",    # University of Auckland
    "site:otago.ac.nz",       # University of Otago
    "site:canterbury.ac.nz",  # University of Canterbury
    "site:vuw.ac.nz",         # Victoria University of Wellington
    "site:massey.ac.nz",      # Massey University
    "site:waikato.ac.nz",     # University of Waikato
    "site:aut.ac.nz",         # Auckland University of Technology
    "site:lincoln.ac.nz",     # Lincoln University
    "site:unitec.ac.nz",      # Unitec Institute of Technology
    "site:niwa.co.nz"         # NIWA (Research Institute)
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for New Zealand."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:auckland.ac.nz", "site:otago.ac.nz", "site:canterbury.ac.nz", "site:nz"]
    for b in broad:
...
class NewZealandScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_new_zealand",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 NZ_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting New Zealand Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="New Zealand Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = NewZealandScraper(verify_ssl=not args.no_verify_ssl)
