...
# -*- coding: utf-8 -*-
"""
Ireland-focused PPTX scraper — Search-Engine-Driven (Europe Series)
Discovers and downloads academic .pptx / .ppt files from Irish institutions (.ie).

Strategies for 10,000+ files:
  1. Priority focus on TCD, UCD, Galway, and DCU.
  2. Broad wildcard targeting across the .ie academic domain.
  3. High native English yield for academic and research materials.
  4. Deep Discovery: Follows HTML pages to find embedded bitstream links in Irish repositories (TARA, etc.).
"""
...
# Irish repositories often use DSpace (TARA, etc.)
BITSTREAM_RE = re.compile(
    r"/(?:bitstream(?:/handle)?/[\d./]+|retrieve/\d+|download/\d+|file|attachment)/[^\"\'\s>{}\[\]\\]+\.pptx?",
    re.IGNORECASE,
)
...
# Irish Academic Topics & Document Types (English)
TOPICS: List[str] = [
    # Document Types
    "lecture slides", "presentation", "research presentation", "course notes",
    "seminar presentation", "workshop materials", "dataset slides",
    
    # Specific Subjects
    "computer science", "artificial intelligence", "machine learning",
    "software engineering", "cybersecurity", "data analytics",
    "pharmaceutical science", "biotechnology", "medicine", "public health",
...
# Major Irish Academic Domains
SITE_DOMAINS: List[str] = [
    "site:tcd.ie",                  # Trinity College Dublin
    "site:ucd.ie",                  # University College Dublin
    "site:universityofgalway.ie",   # Galway
    "site:ucc.ie",                  # Cork
    "site:ul.ie",                   # Limerick
    "site:dcu.ie",                  # Dublin City University
    "site:mu.ie",                   # Maynooth
    "site:tudublin.ie",             # TU Dublin
    "site:hea.ie",                  # Higher Education Authority
    "site:teagasc.ie"               # Agriculture and Food Development Authority
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for Ireland."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:tcd.ie", "site:ucd.ie", "site:ie"]
    for b in broad:
...
class IrelandScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_ireland",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 IE_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def _extract_from_page(self, url: str) -> List[str]:
        """Deep scan a page for embedded pptx links (TARA support)."""
        try:
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Ireland Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Ireland Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = IrelandScraper(verify_ssl=not args.no_verify_ssl)
