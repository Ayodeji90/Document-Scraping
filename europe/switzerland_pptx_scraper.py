...
# -*- coding: utf-8 -*-
"""
Switzerland-focused PPTX scraper — Search-Engine-Driven (Europe Series)
Discovers and downloads academic .pptx / .ppt files from Swiss institutions (.ch).

Strategies for 10,000+ files:
  1. Priority focus on ETH Zurich (ethz.ch) and EPFL (epfl.ch).
  2. Multilingual query matrix (English, German, French).
  3. Deep Discovery: Follows HTML pages to find embedded bitstream links in Swiss repositories.
"""
...
# Swiss repositories often use DSpace or custom systems
BITSTREAM_RE = re.compile(
    r"/(?:bitstream(?:/handle)?/[\d./]+|retrieve/\d+|download/\d+|files)/[^\"\'\s>{}\[\]\\]+\.pptx?",
    re.IGNORECASE,
)
...
# Swiss Academic Topics (Trilingual: EN, DE, FR)
TOPICS: List[str] = [
    # Document Types
    "lecture slides", "presentation", "research presentation",
    "Vorlesung Folien", "Präsentation", "Skript",  # German
    "cours", "diaporama", "présentation", # French
    
    # Specific Subjects
    "computer science", "artificial intelligence", "machine learning",
    "robotics", "robotik", "robotique",
    "physics", "physique", "physik", "quantum mechanics",
    "engineering", "ingenieurwissenschaften", "ingénierie",
    "mathematics", "mathematik", "mathématiques",
    "biology", "biologie", "biotechnology",
    "chemistry", "chemie", "chimie",
    "medicine", "medizin", "médecine",
    "economics", "wirtschaft", "économie",
    "architecture", "architektur", "architecture"
]

# Major Swiss Academic Domains
SITE_DOMAINS: List[str] = [
    "site:ethz.ch",         # ETH Zurich
    "site:epfl.ch",         # EPFL Lausanne
    "site:uzh.ch",          # University of Zurich
    "site:unibas.ch",       # Basel
    "site:unige.ch",        # Geneva
    "site:unibe.ch",        # Bern
    "site:unil.ch",         # Lausanne
    "site:unisg.ch",        # St. Gallen
    "site:fhnw.ch",         # University of Applied Sciences Northwestern Switzerland
    "site:psi.ch",          # Paul Scherrer Institute
    "site:cern.ch",         # CERN (Physics Goldmine)
    "site:zhaw.ch"          # ZHAW Zurich
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for Switzerland."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier
    broad = ["site:ethz.ch", "site:epfl.ch", "site:cern.ch", "site:psi.ch"]
    for b in broad:
...
class SwitzerlandScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_switzerland",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 CH_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Switzerland Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Switzerland Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = SwitzerlandScraper(verify_ssl=not args.no_verify_ssl)
