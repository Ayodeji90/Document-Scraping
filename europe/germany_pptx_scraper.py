...
# -*- coding: utf-8 -*-
"""
Germany-focused PPTX scraper — Search-Engine-Driven (Europe Series)
Discovers and downloads academic .pptx / .ppt files from German institutions.

Strategies for 10,000+ files:
  1. No single academic domain exists in Germany (like .ac.uk), so we target 
     the top 20+ university domains (tum.de, lmu.de, kit.edu, etc.) directly.
  2. Extensive German academic keywords (Vorlesung, Folien, Skript).
  3. Deep Discovery: Follows HTML pages to find embedded bitstream links (DSpace/OPUS).
"""
...
# DSpace/OPUS bitstream matching
BITSTREAM_RE = re.compile(
    r"/(?:bitstream(?:/handle)?/[\d./]+|retrieve/\d+|download/\d+|frontdoor/deliver/index/docId/\d+/file)/[^\"\'\s>{}\[\]\\]+\.pptx?",
    re.IGNORECASE,
)
...
# German Academic Topics & Document Types
TOPICS: List[str] = [
    # Document Types
    "Vorlesung Folien", "Präsentation", "Forschungspräsentation", "Skript", 
    "Seminar", "Kolloquium", "Übung", "lecture slides", "conference presentation",
    
    # Specific Subjects
    "Informatik", "Künstliche Intelligenz", "Maschinenelles Lernen", "Datenbanken",
    "Maschinenbau", "Elektrotechnik", "Bauingenieurwesen", "Verfahrenstechnik",
    "Physik", "Quantenphysik", "Thermodynamik", "Optik", 
    "Chemie", "Organische Chemie", "Physikalische Chemie", 
    "Biologie", "Genetik", "Biotechnologie", 
    "Mathematik", "Statistik", "Algebra", "Analysis",
    "Medizin", "Anatomie", "Physiologie", "Kardiologie", "Neurologie",
    "Wirtschaftswissenschaften", "Betriebswirtschaftslehre", "Volkswirtschaftslehre",
    "Jura", "Rechtswissenschaft", "Psychologie", "Soziologie", "Geschichte"
]

# Major German University & Research Domains
SITE_DOMAINS: List[str] = [
    "site:tum.de",              # TU Munich
    "site:lmu.de",              # LMU Munich
    "site:uni-heidelberg.de",   # Heidelberg
    "site:rwth-aachen.de",      # RWTH Aachen
    "site:kit.edu",             # Karlsruhe Institute of Technology
    "site:hu-berlin.de",        # Humboldt Berlin
    "site:tu-berlin.de",        # TU Berlin
    "site:fu-berlin.de",        # Freie Uni Berlin
    "site:uni-tuebingen.de",    # Tübingen
    "site:uni-freiburg.de",     # Freiburg
    "site:uni-bonn.de",         # Bonn
    "site:uni-frankfurt.de",    # Frankfurt
    "site:uni-goettingen.de",   # Göttingen
    "site:uni-wuerzburg.de",    # Würzburg
    "site:uni-koeln.de",        # Cologne
    "site:tu-dresden.de",       # TU Dresden
    "site:uni-stuttgart.de",    # Stuttgart
    "site:mpg.de",              # Max Planck Society
    "site:fraunhofer.de",       # Fraunhofer Society
    "site:helmholtz.de"         # Helmholtz Association
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for Germany."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top institutions
    for b in SITE_DOMAINS:
        queries.append(f"filetype:pptx {b}")
        queries.append(f"filetype:ppt {b}")
        queries.append(f"Vorlesung Folien filetype:pptx {b}")
...
class GermanyScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_germany",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 DE_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def _extract_from_page(self, url: str) -> List[str]:
        """Deep scan a page for embedded pptx links (OPUS/DSpace support)."""
        try:
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Germany Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Germany Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = GermanyScraper(verify_ssl=not args.no_verify_ssl)
