...
# -*- coding: utf-8 -*-
"""
Finland-focused PPTX scraper — Search-Engine-Driven (Europe Series)
Discovers and downloads academic .pptx / .ppt files from Finnish institutions (.fi).

Strategies for 10,000+ files:
  1. Priority focus on Aalto University, University of Helsinki, and Oulu.
  2. Finnish academic keywords (Luento, Kalvot, Kurssimateriaali).
...
# Finnish repositories often use DSpace or custom portal systems
...
# Finnish Academic Topics & Document Types (Finnish + English)
TOPICS: List[str] = [
    # Document Types
    "luento", "kalvot", "esitys", "kurssimateriaali", "seminaari", "opetus",
    "lecture slides", "presentation", "course materials", "seminar",
    
    # Specific Subjects
    "tietotekniikka", "tekoäly", "koneoppiminen",
    "insinööritieteet", "konetekniikka", "sähkötekniikka", "rakennustekniikka",
    "fysiikka", "kvanttimekaniikka", "termodynamiikka", "optiikka",
    "kemia", "orgaaninen kemia", "biokemia",
    "biologia", "genetiikka", "biotekniikka",
    "matematiikka", "tilastotiede", "analyysi", "algebra",
    "lääketiede", "anatomia", "fysiologia", "kirurgia",
    "taloustiede", "johtaminen", "markkinointi", "rahoitus",
    "historia", "filosofia", "psykologia", "sosiologia", "oikeustiede",
    "arkkitehtuuri", "muotoilu", "kaupunkisuunnittelu"
]

# Major Finnish Academic Domains
SITE_DOMAINS: List[str] = [
    "site:helsinki.fi",      # University of Helsinki
    "site:aalto.fi",         # Aalto University
    "site:utu.fi",           # University of Turku
    "site:jyu.fi",           # University of Jyväskylä
    "site:tuni.fi",          # Tampere University
    "site:uef.fi",           # University of Eastern Finland
    "site:oulu.fi",          # University of Oulu
    "site:abo.fi",           # Åbo Akademi University (Swedish focus)
    "site:lut.fi",           # LUT University
    "site:ulapland.fi"       # University of Lapland
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for Finland."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:helsinki.fi", "site:aalto.fi", "site:oulu.fi", "site:fi"]
    for b in broad:
        queries.append(f"filetype:pptx {b}")
        queries.append(f"filetype:ppt {b}")
        queries.append(f"luento kalvot filetype:pptx {b}")
...
class FinlandScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_finland",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 FI_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Finland Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Finland Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = FinlandScraper(verify_ssl=not args.no_verify_ssl)
