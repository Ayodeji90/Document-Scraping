...
# -*- coding: utf-8 -*-
"""
Nordics-focused PPTX scraper — Search-Engine-Driven (Europe Series)
Discovers and downloads academic .pptx / .ppt files from Sweden, Norway, Denmark, and Finland.

Strategies for 10,000+ files:
  1. Priority focus on KTH, DTU, Aalto, and Karolinska.
  2. High-English proficiency ensures many files are indexed in English.
  3. Nordic academic keywords (Föreläsning, Forelesning, Luento).
  4. Deep Discovery: Follows HTML pages to find embedded bitstream links in Nordic repositories (DiVA, etc.).
"""
...
# Nordic repositories often use DiVA or DSpace
BITSTREAM_RE = re.compile(
    r"/(?:bitstream(?:/handle)?/[\d./]+|retrieve/\d+|download/\d+|file|attachment|smash/get)/[^\"\'\s>{}\[\]\\]+\.pptx?",
    re.IGNORECASE,
)
...
# Nordic Academic Topics & Document Types (English + SE, NO, DK, FI)
TOPICS: List[str] = [
    # Document Types (English + Native)
    "lecture slides", "presentation", "research presentation", "course materials",
    "föreläsning", "presentation", "bildspel",  # Swedish
    "forelesning", "lysbildesett",             # Norwegian
    "forelæsning", "præsentation",             # Danish
    "luento", "esitys",                        # Finnish
    
    # Specific Subjects (English mostly used)
    "computer science", "artificial intelligence", "machine learning",
    "engineering", "robotics", "automation",
    "physics", "quantum mechanics", "nanotechnology",
    "biotechnology", "medical research", "molecular biology",
...
# Major Nordic Academic Domains
SITE_DOMAINS: List[str] = [
    # Sweden
    "site:kth.se", "site:ki.se", "site:uu.se", "site:lu.se", "site:chalmers.se", "site:su.se",
    # Norway
    "site:uio.no", "site:ntnu.no", "site:uib.no", "site:nmbu.no",
    # Denmark
    "site:ku.dk", "site:dtu.dk", "site:au.dk", "site:sdu.dk",
    # Finland
    "site:aalto.fi", "site:helsinki.fi", "site:utu.fi", "site:jyu.fi"
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for the Nordics."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:kth.se", "site:dtu.dk", "site:ntnu.no", "site:aalto.fi", "site:ki.se"]
    for b in broad:
...
class NordicsScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_nordics",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 Nordic_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def _extract_from_page(self, url: str) -> List[str]:
        """Deep scan a page for embedded pptx links (DiVA support)."""
        try:
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Nordics Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Nordics Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = NordicsScraper(verify_ssl=not args.no_verify_ssl)
