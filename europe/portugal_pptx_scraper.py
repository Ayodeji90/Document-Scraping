...
# -*- coding: utf-8 -*-
"""
Portugal-focused PPTX scraper — Search-Engine-Driven (Europe Series)
Discovers and downloads academic .pptx / .ppt files from Portuguese institutions (.pt).

Strategies for 10,000+ files:
  1. Priority focus on University of Lisbon, Porto, and Coimbra.
  2. Portuguese academic keywords (Aulas, Slides, Sebenta).
...
# Portuguese repositories often use DSpace
...
# Portuguese Academic Topics & Document Types (Portuguese + English)
TOPICS: List[str] = [
    # Document Types
    "aulas", "slides", "apresentação", "sebenta", "cadeira",
    "lecture slides", "presentation", "seminário",
    
    # Specific Subjects
    "informática", "inteligência artificial", "aprendizagem automática",
    "engenharia", "mecânica", "electrotecnia", "civil",
    "física", "mecânica quântica", "termodinâmica", "óptica",
    ...
    "arquitectura", "design", "património"
]

# Major Portuguese Academic Domains
SITE_DOMAINS: List[str] = [
    "site:ulisboa.pt",      # University of Lisbon
    "site:up.pt",           # University of Porto
    "site:uc.pt",           # University of Coimbra
    "site:unl.pt",          # Nova University Lisbon
    "site:ua.pt",           # Aveiro
    "site:uminho.pt",       # Minho
    "site:iscte-iul.pt",    # ISCTE Lisbon
    "site:fct.unl.pt",      # Nova School of Science and Technology
    "site:ist.utl.pt",      # Técnico Lisboa
    "site:uporto.pt",       # Porto alias
    "site:reproa.tudor.lu"  # Common European repository link
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for Portugal."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:ulisboa.pt", "site:up.pt", "site:uc.pt", "site:pt"]
    for b in broad:
        queries.append(f"filetype:pptx {b}")
        queries.append(f"filetype:ppt {b}")
        queries.append(f"aulas slides filetype:pptx {b}")
...
class PortugalScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_portugal",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 PT_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Portugal Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Portugal Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = PortugalScraper(verify_ssl=not args.no_verify_ssl)
