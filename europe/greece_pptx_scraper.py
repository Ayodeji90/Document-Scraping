...
# -*- coding: utf-8 -*-
"""
Greece-focused PPTX scraper — Search-Engine-Driven (Europe Series)
Discovers and downloads academic .pptx / .ppt files from Greek institutions (.gr).

Strategies for 10,000+ files:
  1. Priority focus on University of Athens, Aristotle University, and NTUA.
  2. Greek academic keywords (Διάλεξη, Παρουσίαση, Διαφάνειες).
...
# Greek repositories often use DSpace or custom Open Courses portals
BITSTREAM_RE = re.compile(
    r"/(?:bitstream(?:/handle)?/[\d./]+|retrieve/\d+|download/\d+|file|attachment|get|download)/[^\"\'\s>{}\[\]\\]+\.pptx?",
    re.IGNORECASE,
)
...
# Greek Academic Topics & Document Types (Greek + English)
TOPICS: List[str] = [
    # Document Types
    "διάλεξη", "παρουσίαση", "διαφάνειες", "σημειώσεις", "μάθημα",
    "lecture slides", "presentation", "research presentation",
    
    # Specific Subjects
    "πληροφορική", "τεχνητή νοημοσύνη", "μηχανική μάθηση",
    "μηχανική", "μηχανολογία", "ηλεκτρολογία", "πολιτική μηχανική",
    "φυσική", "κβαντομηχανική", "θερμοδυναμική", "οπτική",
    "χημεία", "οργανική χημεία", "βιοχημεία",
    "βιολογία", "γενετική", "βιοτεχνολογία",
    "μαθηματικά", "στατιστική", "άλγεβρα", "ανάλυση",
    "ιατρική", "ανατομία", "φυσιολογία", "χειρουργική",
    "οικονομικά", "διοίκηση", "μάρκετινγκ", "χρηματοοικονομικά",
    "ιστορία", "φιλοσοφία", "ψυχολογία", "κοινωνιολογία", "δίκαιο",
    "αρχιτεκτονική", "σχεδιασμός", "πολεοδομία"
]

# Major Greek Academic Domains
SITE_DOMAINS: List[str] = [
    "site:uoa.gr",           # University of Athens
    "site:auth.gr",          # Aristotle University of Thessaloniki
    "site:ntua.gr",          # National Technical University of Athens
    "site:upatras.gr",       # University of Patras
    "site:uoc.gr",           # University of Crete
    "site:aueb.gr",          # Athens University of Economics and Business
    "site:unipi.gr",         # University of Piraeus
    "site:duth.gr",          # Democritus University of Thrace
    "site:uoi.gr",           # University of Ioannina
    "site:hua.gr"            # Harokopio University of Athens
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for Greece."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:uoa.gr", "site:auth.gr", "site:ntua.gr", "site:gr"]
    for b in broad:
        queries.append(f"filetype:pptx {b}")
        queries.append(f"filetype:ppt {b}")
        queries.append(f"διάλεξη διαφάνειες filetype:pptx {b}")
...
class GreeceScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_greece",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 GR_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Greece Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Greece Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = GreeceScraper(verify_ssl=not args.no_verify_ssl)
