#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Lithuania-focused PPTX scraper — Search-Engine-Driven (Europe Series)"""
...
TOPICS: List[str] = ['paskaita', 'prezentacija', 'mokomoji medžiaga', 'seminaras', 'informatika', 'inžinerija', 'fizika', 'chemija', 'biologija', 'matematika', 'medicina', 'ekonomika', 'teisė']

SITE_DOMAINS: List[str] = ['site:vu.lt', 'site:ktu.lt', 'site:vgtu.lt', 'site:lsmu.lt', 'site:vdu.lt', 'site:ku.lt', 'site:mruni.eu', 'site:lsu.lt', 'site:ism.lt', 'site:ef.vu.lt']
...
def build_query_list() -> List[str]:
    queries = []
    for b in ['site:lt', 'site:vu.lt', 'site:ktu.lt', 'site:vgtu.lt']:
...
class LithuaniaScraper:
    def __init__(self, out_dir="downloaded_ppts_lithuania", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 LT_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Lithuania Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Lithuania Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    LithuaniaScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
