...
# -*- coding: utf-8 -*-
"""
UK-focused PPTX scraper — Search-Engine-Driven (Europe Series)
Discovers and downloads academic .pptx / .ppt files from UK institutions (.ac.uk).

Strategies for 10,000+ files:
  1. Priority focus on Edinburgh DataShare (datashare.ed.ac.uk).
  2. Broad wildcard domains (site:*.ac.uk).
  3. UK-specific English spelling in topics (e.g., paediatrics, behavioural).
  4. Deep Discovery: Follows HTML pages to find embedded bitstream links (DSpace/EPrints).
"""
...
# DSpace/EPrints bitstream matching
BITSTREAM_RE = re.compile(
    r"/(?:bitstream(?:/handle)?/[\d./]+|retrieve/\d+|download/\d+|cgi/users/home\?screen=EPrint::View)|[^\"\'\s>{}\[\]\\]+\.pptx?",
    re.IGNORECASE,
)
...
# UK Academic Topics (incorporating British English spelling)
TOPICS: List[str] = [
    ...
    "data science", "robotics", "cybersecurity", "cloud computing",
    "physics lecture slides", "quantum physics", "thermodynamics", "optics", 
    "chemistry lecture slides", "organic chemistry", "inorganic chemistry", 
    "biology lecture slides", "biotechnology", "microbiology", "genetics", 
    "mathematics lecture slides", "statistics lecture slides", "algebra", "calculus",
    "civil engineering", "electrical engineering", "mechanical engineering", 
    "economics lecture slides", "behavioural economics", "management presentation", 
    "medical lecture slides", "public health", "nursing presentation", "anatomy", 
    "paediatrics", "haematology", "orthopaedics", "gynaecology", "anaesthesia",
    "history", "political science", "sociology", "psychology", "geography", 
    "environmental studies", "law presentation", "constitutional law", "commerce"
]

# Major UK Academic Domains
SITE_DOMAINS: List[str] = [
    "site:ac.uk", "site:gov.uk",
    "site:ox.ac.uk", "site:cam.ac.uk", "site:ucl.ac.uk", "site:imperial.ac.uk",
    "site:ed.ac.uk", "site:manchester.ac.uk", "site:kcl.ac.uk", "site:lse.ac.uk",
    "site:bristol.ac.uk", "site:warwick.ac.uk", "site:leeds.ac.uk", "site:bham.ac.uk",
    "site:shef.ac.uk", "site:gla.ac.uk", "site:soton.ac.uk", "site:nottingham.ac.uk",
    "site:datashare.ed.ac.uk" # Explicit high-priority target
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for the UK."""
    queries: List[str] = []
    
    # 1. Broad Catch-all & Explicit Edinburgh DataShare priority
    broad = ["site:datashare.ed.ac.uk", "site:ac.uk", "site:gov.uk"]
    for b in broad:
        queries.append(f"filetype:pptx {b}")
        queries.append(f"filetype:ppt {b}")
        queries.append(f"dataset materials filetype:pptx {b}")
        queries.append(f"lecture presentation filetype:pptx {b}")

    # 2. Topic × Domain matrix (Broad domains)
    for topic in TOPICS:
        queries.append(f'"{topic}" filetype:pptx site:ac.uk')
        if random.random() < 0.2:
            queries.append(f'"{topic}" filetype:ppt site:ac.uk')
    
    # 3. Specific University Shuffled
    remaining = []
    for domain in SITE_DOMAINS:
        if domain == "site:ac.uk": continue
        for topic in ["lecture slides", "presentation", "course notes", "dataset"]:
...
class UKScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_uk",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 UKScraper/1.0 (Academic Research)"})
        return s
    ...
    def _extract_from_page(self, url: str) -> List[str]:
        """Deep scan a page for embedded pptx links (DSpace/EPrints support)."""
        try:
    ...
    def _download(self, url: str) -> Optional[Path]:
        ...
        try:
            ...
            if dest.stat().st_size < 2097152: # Skip fake files < 5KB
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting UK Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="UK Academic PPTX Scraper (Edinburgh DataShare Focus)")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = UKScraper(verify_ssl=not args.no_verify_ssl)
