#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Serbia-focused PPTX scraper — Search-Engine-Driven (Europe Series)"""
...
TOPICS: List[str] = ['предавање', 'презентација', 'наставни материјали', 'семинар', 'lecture slides', 'presentation', 'course materials', 'рачунарство', 'вештачка интелигенција', 'инжењерство', 'физика', 'хемија', 'биологија', 'математика', 'медицина', 'економија', 'право', 'computer science', 'engineering', 'physics', 'chemistry', 'mathematics', 'medicine']

SITE_DOMAINS: List[str] = ['site:bg.ac.rs', 'site:uns.ac.rs', 'site:ni.ac.rs', 'site:kg.ac.rs', 'site:etf.bg.ac.rs', 'site:matf.bg.ac.rs', 'site:fon.bg.ac.rs', 'site:pmf.uns.ac.rs', 'site:elfak.ni.ac.rs', 'site:ftn.uns.ac.rs']

def build_query_list() -> List[str]:
    queries = []
    for b in ['site:ac.rs', 'site:rs', 'site:bg.ac.rs', 'site:uns.ac.rs']:
        queries.append(f"filetype:pptx {b}")
        queries.append(f"filetype:ppt {b}")
        queries.append(f"предавање презентација filetype:pptx {b}")
    remaining = []
...
class SerbiaScraper:
    def __init__(self, out_dir="downloaded_ppts_serbia", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 RS_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Serbia Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Serbia Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    SerbiaScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
