#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Croatia-focused PPTX scraper — Search-Engine-Driven (Europe Series)"""
...
TOPICS: List[str] = ['predavanje', 'prezentacija', 'nastavni materijali', 'seminar', 'lecture slides', 'presentation', 'course materials', 'računarstvo', 'umjetna inteligencija', 'inženjerstvo', 'fizika', 'kemija', 'biologija', 'matematika', 'medicina', 'ekonomija', 'pravo', 'computer science', 'engineering', 'physics', 'chemistry', 'mathematics', 'medicine']

SITE_DOMAINS: List[str] = ['site:unizg.hr', 'site:unist.hr', 'site:uniri.hr', 'site:unios.hr', 'site:fer.hr', 'site:pmf.unizg.hr', 'site:efzg.hr', 'site:ffzg.hr', 'site:fsb.hr', 'site:riteh.uniri.hr']

def build_query_list() -> List[str]:
    queries = []
    for b in ['site:hr', 'site:unizg.hr', 'site:fer.hr', 'site:unist.hr']:
        queries.append(f"filetype:pptx {b}")
        queries.append(f"filetype:ppt {b}")
        queries.append(f"predavanje prezentacija filetype:pptx {b}")
    remaining = []
...
class CroatiaScraper:
    def __init__(self, out_dir="downloaded_ppts_croatia", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 HR_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Croatia Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Croatia Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    CroatiaScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
