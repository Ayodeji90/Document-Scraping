#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Ukraine-focused PPTX scraper — Search-Engine-Driven (Europe Series)"""
...
TOPICS: List[str] = ['лекція', 'презентація', 'навчальні матеріали', 'семінар', 'курс', 'lecture slides', 'presentation', 'course materials', 'інформатика', 'штучний інтелект', 'інженерія', 'фізика', 'хімія', 'біологія', 'математика', 'медицина', 'економіка', 'право', 'computer science', 'engineering', 'physics', 'chemistry', 'mathematics', 'medicine', 'economics']

SITE_DOMAINS: List[str] = ['site:knu.ua', 'site:kpi.ua', 'site:lnu.edu.ua', 'site:nau.edu.ua', 'site:univ.kiev.ua', 'site:sumdu.edu.ua', 'site:nmu.edu.ua', 'site:onpu.edu.ua', 'site:nure.ua', 'site:lpnu.ua']

def build_query_list() -> List[str]:
    queries = []
    for b in ['site:edu.ua', 'site:ua', 'site:knu.ua', 'site:kpi.ua']:
        queries.append(f"filetype:pptx {b}")
        queries.append(f"filetype:ppt {b}")
        queries.append(f"лекція презентація filetype:pptx {b}")
    remaining = []
...
class UkraineScraper:
    def __init__(self, out_dir="downloaded_ppts_ukraine", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 UA_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Ukraine Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Ukraine Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    UkraineScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
