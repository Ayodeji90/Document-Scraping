...
# -*- coding: utf-8 -*-
"""
Netherlands-focused PPTX scraper — Search-Engine-Driven (Europe Series)
Discovers and downloads academic .pptx / .ppt files from Dutch institutions (.nl).

Strategies for 10,000+ files:
  1. Priority focus on TU Delft, University of Amsterdam, and Utrecht.
  2. High-English proficiency ensures massive volumes of English-language materials.
  3. Dutch academic keywords (Hoorcollege, Presentatie, Dictaat).
  4. Deep Discovery: Follows HTML pages to find embedded bitstream links in Dutch repositories.
"""
...
# Dutch repositories often use DSpace or customized library systems
...
# Dutch Academic Topics & Document Types (English + Dutch)
TOPICS: List[str] = [
    # Document Types
    "lecture slides", "presentation", "research presentation", "course materials",
    "hoorcollege", "presentatie", "slides", "dictaat", "studieverslag",
    
    # Specific Subjects
    "computer science", "artificial intelligence", "machine learning",
    "engineering", "robotics", "aerospace engineering",
    "physics", "quantum computing", "nanoscience",
    "water management", "environmental science", "sustainability",
    "agriculture", "food science", "biology",
    "mathematics", "statistics", "data science",
    "economics", "business", "logistics", "finance",
    "medicine", "public health", "biomedical sciences",
    "law", "international law", "sociology", "psychology",
    "architecture", "urban planning", "design"
]

# Major Dutch Academic Domains
SITE_DOMAINS: List[str] = [
    "site:tudelft.nl",              # TU Delft (Massive source)
    "site:uva.nl",                  # University of Amsterdam
    "site:uu.nl",                   # Utrecht University
    "site:leidenuniversity.nl",     # Leiden
    "site:tue.nl",                  # Eindhoven University of Technology
    "site:wur.nl",                  # Wageningen University & Research
    "site:rug.nl",                  # Groningen
    "site:eur.nl",                  # Erasmus University Rotterdam
    "site:utwente.nl",              # Twente
    "site:vu.nl",                   # Vrije Universiteit Amsterdam
    "site:maastrichtuniversity.nl", # Maastricht
    "site:knaw.nl"                  # Royal Netherlands Academy of Arts and Sciences
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for the Netherlands."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:tudelft.nl", "site:tue.nl", "site:wur.nl", "site:uva.nl"]
    for b in broad:
        queries.append(f"filetype:pptx {b}")
        queries.append(f"filetype:ppt {b}")
        queries.append(f"hoorcollege slides filetype:pptx {b}")
...
class NetherlandsScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_netherlands",
        request_timeout: int = 45,
        delay_seconds: float = 2.0,
        verify_ssl: bool = True,
        max_results_per_query: int = 300, # Depth-First enabled
    ):
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 NL_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Netherlands Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Netherlands Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = NetherlandsScraper(verify_ssl=not args.no_verify_ssl)
