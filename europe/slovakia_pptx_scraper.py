#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Slovakia-focused PPTX scraper — Search-Engine-Driven (Europe Series)"""
...
TOPICS: List[str] = ['prednáška', 'prezentácia', 'študijné materiály', 'seminár', 'informatika', 'inžinierstvo', 'fyzika', 'chémia', 'biológia', 'matematika', 'medicína', 'ekonómia', 'právo']

SITE_DOMAINS: List[str] = ['site:stuba.sk', 'site:uniba.sk', 'site:tuke.sk', 'site:uniza.sk', 'site:ukf.sk', 'site:uniag.sk', 'site:upjs.sk', 'site:umb.sk', 'site:ucm.sk', 'site:tvu.sk']
...
def build_query_list() -> List[str]:
    queries = []
    for b in ['site:sk', 'site:stuba.sk', 'site:uniba.sk', 'site:tuke.sk']:
...
class SlovakiaScraper:
    def __init__(self, out_dir="downloaded_ppts_slovakia", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 SK_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Slovakia Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Slovakia Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    SlovakiaScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
