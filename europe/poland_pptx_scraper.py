...
# -*- coding: utf-8 -*-
"""
Poland-focused PPTX scraper — Search-Engine-Driven (Europe Series)
Discovers and downloads academic .pptx / .ppt files from Polish institutions (.pl).

Strategies for 10,000+ files:
  1. Priority focus on Warsaw University of Technology, AGH, and Jagiellonian University.
  2. Polish academic keywords (Wykład, Prezentacja, Slajdy).
...
# Polish repositories often use DSpace or customized systems (APD, etc.)
BITSTREAM_RE = re.compile(
    r"/(?:bitstream(?:/handle)?/[\d./]+|retrieve/\d+|download/\d+|file|attachment|get|pobierz)/[^\"\'\s>{}\[\]\\]+\.pptx?",
    re.IGNORECASE,
)
...
# Polish Academic Topics & Document Types (Polish + English)
TOPICS: List[str] = [
    # Document Types
    "wykład", "prezentacja", "slajdy", "materiały", "seminarium", "kurs",
    "lecture slides", "presentation", "research presentation",
    
    # Specific Subjects
    "informatyka", "sztuczna inteligencja", "uczenie maszynowe",
    "inżynieria", "mechanika", "elektronika", "budownictwo",
    "fizyka", "mechanika kwantowa", "termodynamika", "optyka",
    "chemia", "chemia organiczna", "biochemia",
    "biologia", "genetyka", "biotechnologia",
    "matematyka", "statystyka", "algebra", "analiza",
    "medycyna", "anatomia", "fizjologia", "chirurgia",
    "ekonomia", "zarządzanie", "marketing", "finanse",
    "historia", "filozofia", "psychologia", "socjologia", "prawo",
    "architektura", "projektowanie", "urbanistyka"
]

# Major Polish Academic Domains
SITE_DOMAINS: List[str] = [
    "site:uw.edu.pl",        # University of Warsaw
    "site:uj.edu.pl",        # Jagiellonian University
    "site:pw.edu.pl",        # Warsaw University of Technology
    "site:agh.edu.pl",       # AGH University of Science and Technology
    "site:uwr.edu.pl",       # University of Wrocław
    "site:amu.edu.pl",       # Adam Mickiewicz University
    "site:pwr.edu.pl",       # Wrocław University of Science and Technology
    "site:p.lodz.pl",        # Lodz University of Technology
    "site:ue.wroc.pl",       # Wroclaw University of Economics
    "site:umcs.pl"           # Marie Curie-Skłodowska University
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for Poland."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:pw.edu.pl", "site:agh.edu.pl", "site:uw.edu.pl", "site:pl"]
    for b in broad:
        queries.append(f"filetype:pptx {b}")
        queries.append(f"filetype:ppt {b}")
        queries.append(f"wykład slajdy filetype:pptx {b}")
...
class PolandScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_poland",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 PL_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Poland Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Poland Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = PolandScraper(verify_ssl=not args.no_verify_ssl)
