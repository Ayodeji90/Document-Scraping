...
# -*- coding: utf-8 -*-
"""
Norway-focused PPTX scraper — Search-Engine-Driven (Europe Series)
Discovers and downloads academic .pptx / .ppt files from Norwegian institutions (.no).

Strategies for 10,000+ files:
  1. Priority focus on NTNU, University of Oslo (UiO), and University of Bergen (UiB).
  2. Norwegian academic keywords (Forelesning, Lysbilder, Kursmateriell).
...
# Norwegian repositories often use DSpace or custom portal systems
...
# Norwegian Academic Topics & Document Types (Norwegian + English)
TOPICS: List[str] = [
    # Document Types
    "forelesning", "lysbilder", "presentasjon", "kursmateriell", "seminar", "notater",
    "lecture slides", "presentation", "course materials", "seminar",
    
    # Specific Subjects
    "informatikk", "kunstig intelligens", "maskinlæring",
    "ingeniørvitenskap", "maskinteknikk", "elektroteknikk", "energiteknikk",
    "fysikk", "kvantemekanikk", "termodynamikk", "optikk",
    "kjemi", "organisk kjemi", "biokjemi",
    "biologi", "genetikk", "bioteknologi",
    "matematikk", "statistikk", "analyse", "algebra",
    "medisin", "anatomi", "fysiologi", "kirurgi",
    "økonomi", "ledelse", "markedsføring", "finans",
    "historie", "filosofi", "psykologi", "sosiologi", "juss",
    "arkitektur", "design", "byplanlegging"
]

# Major Norwegian Academic Domains
SITE_DOMAINS: List[str] = [
    "site:uio.no",            # University of Oslo
    "site:ntnu.no",           # Norwegian University of Science and Technology
    "site:uib.no",            # University of Bergen
    "site:uit.no",            # UiT The Arctic University of Norway
    "site:nmbu.no",           # Norwegian University of Life Sciences
    "site:uis.no",            # University of Stavanger
    "site:uia.no",            # University of Agder
    "site:bi.no",             # BI Norwegian Business School
    "site:nhh.no",            # NHH Norwegian School of Economics
    "site:nord.no"            # Nord University
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for Norway."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:ntnu.no", "site:uio.no", "site:uib.no", "site:no"]
    for b in broad:
        queries.append(f"filetype:pptx {b}")
        queries.append(f"filetype:ppt {b}")
        queries.append(f"forelesning lysbilder filetype:pptx {b}")
...
class NorwayScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_norway",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 NO_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Norway Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Norway Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = NorwayScraper(verify_ssl=not args.no_verify_ssl)
