#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Romania-focused PPTX scraper — Search-Engine-Driven (Europe Series)"""
...
TOPICS: List[str] = ['curs universitar', 'prezentare', 'materiale didactice', 'seminar', 'prelegere', 'lecture slides', 'presentation', 'course materials', 'informatică', 'inteligență artificială', 'inginerie', 'inginerie electrică', 'inginerie civilă', 'fizică', 'chimie', 'biologie', 'matematică', 'medicină', 'economie', 'drept', 'computer science', 'engineering', 'physics', 'chemistry', 'mathematics', 'medicine']

SITE_DOMAINS: List[str] = ['site:ubbcluj.ro', 'site:unibuc.ro', 'site:upb.ro', 'site:uaic.ro', 'site:unitbv.ro', 'site:utcluj.ro', 'site:uvt.ro', 'site:umfcd.ro', 'site:ase.ro', 'site:tuiasi.ro']

def build_query_list() -> List[str]:
    queries = []
    for b in ['site:ro', 'site:ubbcluj.ro', 'site:unibuc.ro', 'site:upb.ro']:
        queries.append(f"filetype:pptx {b}")
        queries.append(f"filetype:ppt {b}")
        queries.append(f"curs prezentare filetype:pptx {b}")
    remaining = []
...
class RomaniaScraper:
    def __init__(self, out_dir="downloaded_ppts_romania", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 RO_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Romania Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Romania Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    RomaniaScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
