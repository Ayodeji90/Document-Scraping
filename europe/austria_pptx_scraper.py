...
# -*- coding: utf-8 -*-
"""
Austria-focused PPTX scraper — Search-Engine-Driven (Europe Series)
Discovers and downloads academic .pptx / .ppt files from Austrian institutions (.at).

Strategies for 10,000+ files:
  1. Priority focus on University of Vienna, TU Wien, and TU Graz.
  2. German academic keywords (Vorlesung, Folien, Skript).
...
# Austrian repositories often use DSpace or Phidra systems
...
# Austrian Academic Topics & Document Types (German + English)
TOPICS: List[str] = [
    # Document Types
    "vorlesung", "folien", "präsentation", "skript", "vortrag",
    "lecture slides", "presentation", "seminar presentation",
    
    # Specific Subjects
    "informatik", "künstliche intelligenz", "maschinelles lernen",
    "ingenieurwesen", "maschinenbau", "elektrotechnik", "bauingenieurwesen",
    "physik", "quantenmechanik", "thermodynamik", "optik",
    "chemie", "organische chemie", "biochemie",
    "biologie", "genetik", "biotechnologie",
    "mathematik", "statistik", "algebra", "analysis",
    "medizin", "anatomie", "physiologie", "chirurgie",
    "wirtschaftswissenschaften", "management", "marketing", "finanzen",
    "geschichte", "philosophie", "psychologie", "soziologie", "recht",
    "architektur", "design", "raumplanung"
]

# Major Austrian Academic Domains
SITE_DOMAINS: List[str] = [
    "site:univie.ac.at",     # University of Vienna
    "site:tuwien.at",        # TU Wien
    "site:tugraz.at",       # Graz University of Technology
    "site:uibk.ac.at",      # University of Innsbruck
    "site:jku.at",           # Johannes Kepler University Linz
    "site:plus.ac.at",       # University of Salzburg
    "site:wu.ac.at",         # Vienna University of Economics and Business
    "site:meduniwien.ac.at", # Medical University of Vienna
    "site:unileoben.ac.at",  # Montanuniversität Leoben (Engineering Hub)
    "site:boku.ac.at"        # University of Natural Resources and Life Sciences
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for Austria."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:univie.ac.at", "site:tuwien.at", "site:tugraz.at", "site:at"]
    for b in broad:
...
class AustriaScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_austria",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 AT_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Austria Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Austria Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = AustriaScraper(verify_ssl=not args.no_verify_ssl)
