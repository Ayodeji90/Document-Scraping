#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Slovenia-focused PPTX scraper — Search-Engine-Driven (Europe Series)"""
...
TOPICS: List[str] = ['predavanje', 'predstavitev', 'študijsko gradivo', 'seminar', 'informatika', 'inženirstvo', 'fizika', 'kemija', 'biologija', 'matematika', 'medicina', 'ekonomija', 'pravo']

SITE_DOMAINS: List[str] = ['site:uni-lj.si', 'site:um.si', 'site:upr.si', 'site:feri.um.si', 'site:fe.uni-lj.si', 'site:fmf.uni-lj.si', 'site:ef.uni-lj.si', 'site:fri.uni-lj.si', 'site:fs.uni-lj.si', 'site:pf.uni-lj.si']
...
def build_query_list() -> List[str]:
    queries = []
    for b in ['site:si', 'site:uni-lj.si', 'site:um.si', 'site:upr.si']:
...
class SloveniaScraper:
    def __init__(self, out_dir="downloaded_ppts_slovenia", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 SI_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Slovenia Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Slovenia Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    SloveniaScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
