...
# -*- coding: utf-8 -*-
"""
Italy-focused PPTX scraper — Search-Engine-Driven (Europe Series)
Discovers and downloads academic .pptx / .ppt files from Italian institutions (.it).

Strategies for 10,000+ files:
  1. Priority focus on major hubs: Bologna (unibo.it), Sapienza (uniroma1.it), Polimi (polimi.it).
  2. Italian academic keywords (Lezioni, Slide, Dispense).
  3. Deep Discovery: Follows HTML pages to find embedded bitstream links in Italian repositories.
"""
...
# Italian repositories often use DSpace or customized portal systems
BITSTREAM_RE = re.compile(
    r"/(?:bitstream(?:/handle)?/[\d./]+|retrieve/\d+|download/\d+|file|allegato)/[^\"\'\s>{}\[\]\\]+\.pptx?",
    re.IGNORECASE,
)
...
# Italian Academic Topics & Document Types
TOPICS: List[str] = [
    # Document Types
    "lezioni", "slide", "presentazione", "dispense", "corso",
    "lecture slides", "presentation", "seminar",
    
    # Specific Subjects
    "informatica", "intelligenza artificiale", "machine learning",
    "ingegneria", "meccanica", "elettronica", "civile",
    "fisica", "meccanica quantistica", "termodinamica", "ottica",
    "chimica", "chimica organica", "biochimica",
    "biologia", "genetica", "biotecnologia",
    "matematica", "statistica", "algebra", "analisi",
    "medicina", "anatomia", "fisiologia", "chirurgia",
    "economia", "gestione", "marketing", "finanza",
    "storia", "filosofia", "psicologia", "sociologia", "diritto",
    "architettura", "design", "beni culturali"
]

# Major Italian Academic Domains
SITE_DOMAINS: List[str] = [
    "site:unibo.it",        # Bologna
    "site:uniroma1.it",     # Sapienza Rome
    "site:polimi.it",       # Politecnico di Milano
    "site:unipd.it",        # Padua
    "site:polito.it",       # Politecnico di Torino
    "site:unina.it",        # Naples
    "site:unimi.it",        # Milan
    "site:unifi.it",        # Florence
    "site:unipisa.it",      # Pisa
    "site:sns.it",          # Scuola Normale Superiore
    "site:cnr.it",          # National Research Council (CNR)
    "site:cineca.it"        # CINECA
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for Italy."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier
    broad = ["site:unibo.it", "site:polimi.it", "site:it", "site:cnr.it"]
    for b in broad:
        queries.append(f"filetype:pptx {b}")
        queries.append(f"filetype:ppt {b}")
        queries.append(f"lezioni slide filetype:pptx {b}")
        queries.append(f"presentazione filetype:pptx {b}")
...
class ItalyScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_italy",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 IT_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Italy Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Italy Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = ItalyScraper(verify_ssl=not args.no_verify_ssl)
