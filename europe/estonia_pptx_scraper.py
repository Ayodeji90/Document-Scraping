#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Estonia-focused PPTX scraper — Search-Engine-Driven (Europe Series)"""
...
TOPICS: List[str] = ['loeng', 'esitlus', 'õppematerjalid', 'seminar', 'informaatika', 'inseneeria', 'füüsika', 'keemia', 'bioloogia', 'matemaatika', 'meditsiin', 'majandus', 'õigus']

SITE_DOMAINS: List[str] = ['site:ut.ee', 'site:taltech.ee', 'site:tlu.ee', 'site:emu.ee', 'site:ebs.ee', 'site:artun.ee', 'site:cs.ut.ee', 'site:kul.ee']
...
def build_query_list() -> List[str]:
    queries = []
    for b in ['site:ee', 'site:ut.ee', 'site:taltech.ee', 'site:tlu.ee']:
...
class EstoniaScraper:
    def __init__(self, out_dir="downloaded_ppts_estonia", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 EE_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Estonia Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Estonia Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    EstoniaScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
