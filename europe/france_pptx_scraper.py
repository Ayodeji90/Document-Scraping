...
# -*- coding: utf-8 -*-
"""
France-focused PPTX scraper — Search-Engine-Driven (Europe Series)
Discovers and downloads academic .pptx / .ppt files from French institutions and open archives.

Strategies for 10,000+ files:
  1. Priority focus on HAL open science archive (hal.science / archives-ouvertes.fr).
  2. Targeted domains for major French universities and research institutes (CNRS, Inria).
  3. Extensive French academic keywords (diaporama, cours, soutenance).
...
# HAL / DSpace bitstream matching
BITSTREAM_RE = re.compile(
    r"/(?:bitstream(?:/handle)?/[\d./]+|retrieve/\d+|download/\d+|document)/[^\"\'\s>{}\[\]\\]+\.pptx?",
    re.IGNORECASE,
)
...
# French Academic Topics & Document Types
TOPICS: List[str] = [
    # Document Types
    "diaporama", "diapositives", "présentation", "cours", "soutenance", 
    "TD", "travaux dirigés", "séminaire", "colloque", "lecture slides",
    
    # Specific Subjects
    "informatique", "intelligence artificielle", "apprentissage automatique", "données",
    "ingénierie", "génie mécanique", "génie civil", "génie électrique",
    "physique", "mécanique quantique", "thermodynamique", "optique", 
    "chimie", "chimie organique", "biochimie", 
    "biologie", "génétique", "biotechnologie", "écologie",
    "mathématiques", "statistiques", "algèbre", "analyse",
    "médecine", "anatomie", "physiologie", "santé publique",
    "économie", "gestion", "finance", "marketing",
    "droit", "droit public", "droit civil", "sciences politiques",
    "histoire", "sociologie", "psychologie", "philosophie", "littérature"
]

# Major French University & Research Domains
SITE_DOMAINS: List[str] = [
    "site:hal.science",                     # HAL Open Archive (Massive)
    "site:archives-ouvertes.fr",            # HAL Open Archive Legacy
    "site:psl.eu",                          # Paris Sciences et Lettres
    "site:sorbonne-universite.fr",          # Sorbonne
    "site:universite-paris-saclay.fr",      # Paris-Saclay
    "site:univ-grenoble-alpes.fr",          # Grenoble Alpes
    "site:ens-lyon.fr",                     # ENS Lyon
    "site:pantheonsorbonne.fr",             # Panthéon-Sorbonne
    "site:univ-amu.fr",                     # Aix-Marseille
    "site:unistra.fr",                      # Strasbourg
    "site:u-bordeaux.fr",                   # Bordeaux
    "site:univ-lille.fr",                   # Lille
    "site:insa-lyon.fr",                    # INSA Lyon
    "site:cnrs.fr",                         # CNRS
    "site:inria.fr",                        # Inria
    "site:enseignementsup-recherche.gouv.fr" # Gov Research Portal
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for France."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top institutions (HAL is the goldmine)
    broad = ["site:hal.science", "site:archives-ouvertes.fr", "site:cnrs.fr", "site:inria.fr"]
    for b in broad:
        queries.append(f"filetype:pptx {b}")
        queries.append(f"filetype:ppt {b}")
        queries.append(f"diaporama cours filetype:pptx {b}")
        queries.append(f"présentation filetype:pptx {b}")
...
class FranceScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_france",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 FR_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def _extract_from_page(self, url: str) -> List[str]:
        """Deep scan a page for embedded pptx links (HAL support)."""
        try:
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting France Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="France Academic PPTX Scraper (HAL Focus)")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = FranceScraper(verify_ssl=not args.no_verify_ssl)
