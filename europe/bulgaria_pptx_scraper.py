#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Bulgaria-focused PPTX scraper — Search-Engine-Driven (Europe Series)"""
...
TOPICS: List[str] = ['лекция', 'презентация', 'учебни материали', 'семинар', 'информатика', 'инженерство', 'физика', 'химия', 'биология', 'математика', 'медицина', 'икономика', 'право']

SITE_DOMAINS: List[str] = ['site:uni-sofia.bg', 'site:tu-sofia.bg', 'site:uni-plovdiv.bg', 'site:uni-vt.bg', 'site:nbu.bg', 'site:aubg.edu', 'site:mu-sofia.bg', 'site:tu-varna.bg', 'site:btu.bg', 'site:unwe.bg']
...
def build_query_list() -> List[str]:
    queries = []
    for b in ['site:bg', 'site:uni-sofia.bg', 'site:tu-sofia.bg', 'site:uni-plovdiv.bg']:
...
class BulgariaScraper:
    def __init__(self, out_dir="downloaded_ppts_bulgaria", request_timeout=45, delay_seconds=2.0, verify_ssl=True, max_results_per_query=300):
    ...
    def _build_session(self):
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 BG_Scraper/1.0 (Academic Research)"}); return s
    ...
    def scrape(self, target=10000, follow_pages=True):
        ...
        logger.info("Starting Bulgaria Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Bulgaria Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true")
    args = parser.parse_args()
    BulgariaScraper(verify_ssl=not args.no_verify_ssl).scrape(target=args.target, follow_pages=not args.no_follow)
