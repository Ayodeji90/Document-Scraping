...
# -*- coding: utf-8 -*-
"""
Belgium-focused PPTX scraper — Search-Engine-Driven (Europe Series)
Discovers and downloads academic .pptx / .ppt files from Belgian institutions (.be).

Strategies for 10,000+ files:
  1. Priority focus on KU Leuven, Ghent University, and UC Louvain.
  2. Trilingual query matrix (English, Dutch, French).
  3. Deep Discovery: Follows HTML pages to find embedded bitstream links in Belgian repositories.
"""
...
# Belgian repositories often use DSpace or custom portal systems
...
# Belgian Academic Topics & Document Types (English + Dutch + French)
TOPICS: List[str] = [
    # Document Types
    "lecture slides", "presentation", "research presentation",
    "hoorcollege", "presentatie", "slides",  # Dutch
    "cours", "diaporama", "présentation", # French
    
    # Specific Subjects
    "computer science", "artificial intelligence", "machine learning",
    "engineering", "robotics", "nanotechnology",
    "physics", "quantum mechanics", "biotechnology",
...
# Major Belgian Academic Domains
SITE_DOMAINS: List[str] = [
    "site:kuleuven.be",     # KU Leuven (Massive source)
    "site:ugent.be",        # Ghent University
    "site:uclouvain.be",    # UC Louvain
    "site:vub.be",          # Vrije Universiteit Brussel
    "site:ulb.be",          # Université Libre de Bruxelles
    "site:uantwerpen.be",   # Antwerp
    "site:uliege.be",       # Liège
    "site:umons.ac.be",     # Mons
    "site:uhasselt.be",     # Hasselt
    "site:unamur.be"        # Namur
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for Belgium."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:kuleuven.be", "site:ugent.be", "site:uclouvain.be"]
    for b in broad:
...
class BelgiumScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_belgium",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 BE_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Belgium Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Belgium Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = BelgiumScraper(verify_ssl=not args.no_verify_ssl)
