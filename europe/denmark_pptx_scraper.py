...
# -*- coding: utf-8 -*-
"""
Denmark-focused PPTX scraper — Search-Engine-Driven (Europe Series)
Discovers and downloads academic .pptx / .ppt files from Danish institutions (.dk).

Strategies for 10,000+ files:
  1. Priority focus on DTU, KU, and AU.
  2. Danish academic keywords (Forelæsning, Slides, Kursusmateriale).
...
# Danish repositories often use DSpace or custom portal systems
...
# Danish Academic Topics & Document Types (Danish + English)
TOPICS: List[str] = [
    # Document Types
    "forelæsning", "slides", "præsentation", "kursusmateriale", "seminar",
    "lecture slides", "presentation", "course materials", "seminar",
    
    # Specific Subjects
    "datalogi", "kunstig intelligens", "maskinlæring",
    "ingeniørvidenskab", "maskinteknik", "elektroteknik", "byggeteknik",
    "fysik", "kvantemekanik", "termodynamik", "optik",
    "kemi", "organisk kemi", "biokemi",
    "biologi", "genetik", "bioteknologi",
    "matematik", "statistik", "calculus", "algebra",
    "medicin", "anatomi", "fysiologi", "kirurgi",
    "økonomi", "ledelse", "marketing", "finans",
    "historie", "filosofi", "psykologi", "sociologi", "jura",
    "arkitektur", "design", "byplanlægning"
]

# Major Danish Academic Domains
SITE_DOMAINS: List[str] = [
    "site:ku.dk",            # University of Copenhagen
    "site:au.dk",            # Aarhus University
    "site:dtu.dk",           # Technical University of Denmark
    "site:sdu.dk",           # University of Southern Denmark
    "site:aau.dk",           # Aalborg University
    "site:itu.dk",           # IT University of Copenhagen
    "site:cbs.dk",           # Copenhagen Business School
    "site:ruc.dk",           # Roskilde University
    "site:adm.ku.dk",        # KU Administration
    "site:regionsyddanmark.dk" # Regional Health/Research
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for Denmark."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:ku.dk", "site:au.dk", "site:dtu.dk", "site:dk"]
    for b in broad:
        queries.append(f"filetype:pptx {b}")
        queries.append(f"filetype:ppt {b}")
        queries.append(f"forelæsning slides filetype:pptx {b}")
...
class DenmarkScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_denmark",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 DK_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Denmark Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Denmark Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = DenmarkScraper(verify_ssl=not args.no_verify_ssl)
