...
# -*- coding: utf-8 -*-
"""
Czech Republic-focused PPTX scraper — Search-Engine-Driven (Europe Series)
Discovers and downloads academic .pptx / .ppt files from Czech institutions (.cz).

Strategies for 10,000+ files:
  1. Priority focus on Charles University, ČVUT, and Masaryk University.
  2. Czech academic keywords (Přednáška, Prezentace, Slajdy).
...
# Czech repositories often use DSpace or custom portal systems
BITSTREAM_RE = re.compile(
    r"/(?:bitstream(?:/handle)?/[\d./]+|retrieve/\d+|download/\d+|file|attachment|get|stahnout)/[^\"\'\s>{}\[\]\\]+\.pptx?",
    re.IGNORECASE,
)
...
# Czech Academic Topics & Document Types (Czech + English)
TOPICS: List[str] = [
    # Document Types
    "přednáška", "prezentace", "slajdy", "materiály", "seminář", "kurs",
    "lecture slides", "presentation", "research presentation",
    
    # Specific Subjects
    "informatika", "umělá inteligence", "strojové učení",
    "inženýrství", "mechanika", "elektronika", "stavitelství",
    "fyzika", "kvantová mechanika", "termodynamika", "optika",
    "chemie", "organická chemie", "biochemie",
    "biologie", "genetika", "biotechnologie",
    "matematika", "statistika", "algebra", "analýza",
    "medicína", "anatomie", "fyziologie", "chirurgie",
    "ekonomie", "management", "marketing", "finance",
    "historie", "filozofie", "psychologie", "sociologie", "právo",
    "architektura", "design", "urbanismus"
]

# Major Czech Academic Domains
SITE_DOMAINS: List[str] = [
    "site:cuni.cz",          # Charles University (Prague)
    "site:cvut.cz",          # Czech Technical University (Prague)
    "site:muni.cz",          # Masaryk University (Brno)
    "site:vse.cz",           # University of Economics and Business (Prague)
    "site:vut.cz",           # Brno University of Technology
    "site:upol.cz",          # Palacký University Olomouc
    "site:zcu.cz",           # University of West Bohemia
    "site:osu.cz",           # University of Ostrava
    "site:vscht.cz",         # University of Chemistry and Technology Prague
    "site:czu.cz"            # Czech University of Life Sciences Prague
]

def build_query_list() -> List[str]:
    """Return a massive list of prioritized queries for the Czech Republic."""
    queries: List[str] = []
    
    # 1. Broad Catch-all for top tier institutions
    broad = ["site:cuni.cz", "site:cvut.cz", "site:muni.cz", "site:cz"]
    for b in broad:
        queries.append(f"filetype:pptx {b}")
        queries.append(f"filetype:ppt {b}")
        queries.append(f"přednáška slajdy filetype:pptx {b}")
...
class CzechRepublicScraper:
    def __init__(
        self,
        out_dir: str = "downloaded_ppts_czech_republic",
    ...
    def _build_session(self) -> requests.Session:
        ...
        s.headers.update({"User-Agent": "Mozilla/5.0 CZ_Scraper/1.0 (Academic Research)"})
        return s
    ...
    def scrape(self, target: int = 10000, follow_pages: bool = True):
        ...
        logger.info("Starting Czech Republic Scale-Up: Target=%d, Queries=%d", target, len(queries))
...
def main():
    parser = argparse.ArgumentParser(description="Czech Republic Academic PPTX Scraper")
    parser.add_argument("--target", type=int, default=10000)
    parser.add_argument("--no-verify-ssl", action="store_true")
    parser.add_argument("--no-follow", action="store_true", help="Disable deep page scanning")
    args = parser.parse_args()

    scraper = CzechRepublicScraper(verify_ssl=not args.no_verify_ssl)
