#!/usr/bin/env python3
"""
Delivery packager for CKAN scraper output — BATCH_03.

Deliberately independent from deliver_to_gdrive.py (which is BATCH_02 and
scans the region folders + joint_downloaded/ shared by mega/commoncrawl/
wayback): this only scans ckan_downloaded/, so CKAN's delivery, numbering,
and Drive destination stay fully separate from the rest of the pipeline.

Usage:
    python deliver_ckan_to_gdrive.py                    # one-shot
    python deliver_ckan_to_gdrive.py --watch             # keep running
    python deliver_ckan_to_gdrive.py --gdrive-path gdrive_staging_ckan
"""
import argparse
import json
import re
...
from src.utils.persistence import get_next_batch_number

BATCH_ID = "BATCH_03"
BATCH_NAME_RE = re.compile(rf'^{BATCH_ID}_names_\d+\.(pptx|ppt)$', re.IGNORECASE)
...
MIN_SLIDES = 5
SOURCE_DIR = Path("ckan_downloaded")
DELIVERED_LOG = Path("logs/delivered_batch03.txt")
...
def process_file(
    ...
    if BATCH_NAME_RE.match(filepath.name):
        dest_name = filepath.name
    else:
        num = get_next_batch_number(BATCH_ID)
        dest_name = f"{BATCH_ID}_names_{num:06d}{filepath.suffix.lower() or '.pptx'}"
...
def scan_and_deliver(gdrive_path: Path) -> int:
    ...
    files = list(SOURCE_DIR.glob("*.pptx")) + list(SOURCE_DIR.glob("*.ppt"))
    total = 0
...
def main():
    parser = argparse.ArgumentParser(description=f"Deliver validated CKAN PPTXs to {BATCH_ID}")
    parser.add_argument("--gdrive-path", default="gdrive_staging_ckan")
    parser.add_argument("--watch", action="store_true", help="Keep running, check every --interval seconds")
    parser.add_argument("--interval", type=int, default=120, help="Seconds between checks in watch mode")
    ...
    if args.watch:
        ...
        n = scan_and_deliver(gdrive_path)
        total_in_drive = sum(1 for _ in (gdrive_path / "files").glob("*"))
        logger.info("Done — %d delivered. Total in %s: %d", n, BATCH_ID, total_in_drive)
